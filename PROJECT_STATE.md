...
- Estado mais recente: o código local inclui afectação explícita de provisões a trabalhos a preço fixo, no pagamento ou a partir de saldo anterior, com leitura dos abatimentos nos painéis e em «Por receber». A migration e o pgTAP de 19 verificações continuam por aplicar/executar. Segurança de ficheiros, lint, tipos, 228 testes unitários, build e dry-run Cloudflare passaram. Nos E2E, 118 cenários passaram, quatro falharam pelo mesmo mock desactualizado e três foram ignorados; o mock foi corrigido e os oito testes de provisões passaram na repetição. A CI do PR #44 passou em 18-09-2026 (validação, dependências e scan); o Supabase Preview foi ignorado. A segunda branch temporária Supabase falhou ao reproduzir migrações antigas após a oitava, foi apagada e confirmou-se apenas `main`. A repartição LEGALTEAM e a emissão de documentos ainda não estão integradas com os trabalhos a preço fixo. Nenhuma escrita em produção. Apesar da ordem «publica», a promoção aguarda o ensaio obrigatório da migration num esquema actual e a conclusão destas integrações financeiras.
...
- Auditoria de 18-09-2026: a branch local já prepara a soma do preço fixo nos dashboards de cliente, sociedade e responsável, nos gráficos, nas páginas de entrada e nas categorias de cliente (repartidas pelo perfil de cada registo). Os RPCs antigos continuam a agregar os registos, e esta composição ainda não foi reconciliada numa base actual de ensaio. Provisões, documentos e LEGALTEAM também continuam pendentes; não publicar esta versão como funcionalidade financeira concluída.
- Provisões sobre trabalhos a preço fixo: por decisão do utilizador, a aplicação terá de perguntar ao registar a provisão se deseja afectar montantes a trabalhos já criados, escolhendo cada trabalho e montante, ou deixar tudo disponível para mais tarde. Os honorários acordados antes de IVA e o valor médio/hora não diminuem. A arquitectura e o texto da pergunta estão em `docs/fixed-fee-reconciliation.md`; a ligação transaccional ao livro de provisões e a interface ainda não estão implementadas.
- Decisão contabilística de 18-09-2026: o preço acordado do trabalho é honorário antes de IVA. As médias e os painéis de honorários usam essa base; uma provisão monetária abate ao total com IVA e exige repartição entre base e imposto. A afectação ainda não está implementada.
- A branch temporária Supabase criada para ensaio em 18-09-2026 ficou presa ao esquema de Agosto (8 migrations) e foi apagada de imediato para terminar o custo horário; confirmou-se que só `main` continua na lista de branches. A migration de preço fixo permanece não aplicada. É necessário um ambiente de teste isolado que reproduza a base actual.
- A fase operacional do trabalho é visível na lista e no detalhe da ficha, com filtro «Todos / Por iniciar / Em curso / Terminados / Cancelados». Começa «Por iniciar», muda automaticamente com o primeiro registo associado e só «Terminado» é assinalado manualmente. Não altera a facturação nem o pagamento.
...
- `package.json`, `public/release-notes.json` e `docs/deployment.md` indicam 0.10.20. A aplicação online e as notas de versão foram confirmadas novamente em 0.10.20. Deployment e version ID abaixo são os registados no handover do GitHub; o painel Cloudflare não foi consultado nesta verificação por exigir autenticação.

## Release 0.10.20 publicada — 16-09-2026

- Branch funcional `codex/fix-paid-invoice-attention-0.10.20`; PR #35 fundido em GitHub `main` no commit `82a0b35b13734c00701dbd5eac62d352aa80f5e7`. Produção Cloudflare 0.10.20 confirmada desde `2026-09-16 10:42:47 UTC`: `https://legal-carina.dabranches.workers.dev`, deployment `af4dd533-6daf-4efe-af87-dd811504c82c`, version `bb3d809a-e94b-45ca-be7c-a8e3f6546dd3`. Rollback frontend 0.10.19: `e0a0442b-c268-4e3d-a00c-0f759708ff19`.
...
- A navegação móvel deixa de ser tapada pela pesquisa dos clientes e os controlos cabem dentro da caixa a 320 px. Regressão nos 11 modelos de iPhone em claro e escuro; 121 E2E locais aprovados/3 condicionais ignorados, 207 testes unitários, lint, tipos, segurança, build, dry-run e CI `35077174048` verdes. Sem escrita na base de dados, Auth ou permissões.
...
- As listas de clientes Particulares, Empresas e Mistos abrem em caixas compactas por defeito; permitem regressar à tabela. A pesquisa, a alternância e a criação partilham uma linha fixa. Cobrança activa fica vermelha e indisponível fica verde suave; os botões conservam os rótulos Ficha/Nota/Cobrança, com explicações apenas ao passar o rato.
...
- O browser integrado local autenticado confirmou a versão 0.10.19, os valores de avença reais e a distinção entre provisão e nota. Produção e notas acumuladas também confirmadas por HTTP após o deploy.
