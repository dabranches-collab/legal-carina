...
- Estado mais recente: o código local inclui afectação explícita de provisões a trabalhos a preço fixo, no pagamento ou a partir de saldo anterior, com leitura dos abatimentos nos painéis e em «Por receber». A migration e o pgTAP de 19 verificações continuam por aplicar/executar. Segurança de ficheiros, lint, tipos, 228 testes unitários, build e dry-run Cloudflare passaram. Nos E2E, 118 cenários passaram, quatro falharam pelo mesmo mock desactualizado e três foram ignorados; o mock foi corrigido e os oito testes de provisões passaram na repetição. A segunda branch temporária Supabase falhou ao reproduzir migrações antigas após a oitava, foi apagada e confirmou-se apenas `main`. Nenhuma escrita em produção. Apesar da ordem «publica», a promoção aguarda o ensaio obrigatório da migration num esquema actual e CI verde.
