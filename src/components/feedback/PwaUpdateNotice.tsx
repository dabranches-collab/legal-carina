...
export function PwaUpdateNotice() {
  ...
  const sameVersion=release?.version===installedNotes.version
  const availableChanges=release?(sameVersion?release.changes:changesSince(release,installedNotes.version)):[]
  return <aside role="status" className="app-safe-toast fixed z-[70] mx-auto max-h-[70dvh] max-w-md overflow-y-auto rounded-xl border border-border bg-surface p-4 shadow-raised"><p className="font-semibold">Actualização disponível{release?` · ${release.version}`:''}</p>{release&&<><p className="mt-2 text-sm font-semibold">{sameVersion?'O que muda nesta versão:':`O que muda desde ${installedNotes.version}:`}</p><ul className="mt-2 list-disc space-y-1 pl-5 text-sm">{availableChanges.map((change,index)=><li key={index}>{change}</li>)}</ul></>}<p className="mt-2 text-sm text-text-secondary">Actualize quando for conveniente. O trabalho actual deve estar guardado.</p><button className="mt-3 min-h-11 rounded-lg bg-primary px-4 font-semibold text-surface" onClick={() => {
    try{if(!sameVersion)localStorage.setItem('carina-release-notes-from',installedNotes.version)}catch{/* Apenas registo local da versão de origem. */}
