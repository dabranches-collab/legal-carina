...
interface MetricCardProps { label: string; value: string; detail: string; trend?: string; tone?: 'default' | 'success' | 'warning' | 'danger'; icon: IconName }
...
export function MetricCard({ label, value, detail, trend, tone = 'default', icon }: MetricCardProps) {
  ...
  return <article className="card group min-w-0 p-4 transition-[transform,box-shadow] hover:-translate-y-0.5 hover:shadow-raised">
    ...
    <p className="mt-4 truncate text-xs font-medium text-text-secondary">{label}</p><p className="mt-1 text-xl font-semibold tabular-nums tracking-tight">{value}</p><p className="mt-1 truncate text-xs text-text-secondary">{detail}</p>
  </article>
}
