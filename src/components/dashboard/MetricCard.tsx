...
interface MetricCardProps { label: string; value: string; detail: string; trend?: string; tone?: 'default' | 'success' | 'warning' | 'danger'; icon: IconName; financial?: boolean; detailHref?: string }

export function MetricCard({ label, value, detail, trend, tone = 'default', icon, financial = false, detailHref }: MetricCardProps) {
  ...
  return <article className="card group min-w-0 p-4 transition-[transform,box-shadow] hover:-translate-y-0.5 hover:shadow-raised">
    ...
    {detailHref&&<a href={detailHref} onClick={openDetail} aria-label={`Abrir movimentos de ${label}`} className="mt-3 flex min-h-9 items-center justify-center gap-2 rounded-lg bg-secondary px-3 text-xs font-semibold text-surface shadow-sm hover:brightness-110"><Icon name="eye" className="size-4"/>Ver movimentos</a>}
  </article>
}
