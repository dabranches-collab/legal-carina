...
import { WorkResultsLink } from '../../features/work-entries/WorkResultsLink'
...
export function MetricCard({ label, value, detail, trend, tone = 'default', icon, financial = false, detailHref, subtotals=[] }: MetricCardProps) {
  ...
  return <article className={`card group flex min-w-0 flex-col p-4 transition-[transform,box-shadow] hover:-translate-y-0.5 hover:shadow-raised ${tone==='danger'?'card-danger':''}`}>
    ...
    <p className={`${financial?'financial-value ':''}metric-card-value mt-2 text-xl font-semibold tabular-nums tracking-tight`}>{value}</p><p className={`${financial&&detail.includes('%')?'financial-value ':''}metric-card-detail mt-1 min-h-8 text-xs text-text-secondary`}>{detail}</p>
    ...
    {detailHref&&<div className="mt-auto pt-3"><WorkResultsLink label={label} href={detailHref} aria-label={`Abrir movimentos de ${label}`} className={`flex min-h-9 items-center justify-center gap-2 rounded-lg px-3 text-xs font-semibold text-surface shadow-sm hover:brightness-110 ${tone==='danger'?'bg-danger':tone==='success'?'bg-success':'bg-secondary'}`}><Icon name="eye" className="size-4"/>Ver movimentos</WorkResultsLink></div>}
  </article>
}
