...
export function AppShell({ activeView, selectedSociety, selectedProfessional, selectedClientType, selectedClientMode, settingsEntity, onRefresh, onNavigate, onNavigateSociety, onNavigateProfessional, onNavigateClientType, onNavigateSettings, children }: AppShellProps) {
  ...
  useEffect(()=>{
    const activeMenu=activeView==='billing'||activeView==='professionals'||activeView==='clients'
      ? activeView
      : activeView==='admin'||activeView==='admin-users'||activeView==='master-data'||activeView==='imports'||activeView==='import-review'
        ? 'admin'
        : null
    setExpandedMenu(activeMenu)
    setAdministrationExpanded(activeView==='admin'||activeView==='admin-users'||activeView==='imports'||activeView==='import-review')
  },[activeView])
  ...
  const isNavigationSelected = (id:ViewId) => activeView===id || (id==='admin'&&(activeView==='admin-users'||activeView==='master-data'||activeView==='imports'||activeView==='import-review'))
      ...
      <aside aria-label="Navegação principal" className={`app-shell-sidebar fixed inset-y-0 left-0 z-40 flex flex-col border-r border-border bg-navigation text-navigation-text transition-[width,transform] duration-200 ${collapsed ? 'lg:w-20' : 'lg:w-64'} w-[min(18rem,calc(100vw-var(--safe-right)))] ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="scrollbar-thin flex-1 overflow-y-auto px-3 py-4">
          <ul className="space-y-1">
            {navigation.filter(item=>item.id!=='admin'||canManageSettings).map((item) => {const selected=isNavigationSelected(item.id);const hasSubmenu=item.id==='billing'||item.id==='professionals'||item.id==='clients'||item.id==='admin';const expanded=expandedMenu===item.id;return <li key={item.id}><button type="button" title={collapsed ? menuLabel(item.label) : undefined} onClick={() => { if(hasSubmenu)setExpandedMenu(value=>value===item.id?null:item.id);else setExpandedMenu(null);onNavigate(item.id) }} aria-current={selected ? 'page' : undefined} aria-expanded={hasSubmenu?expanded:undefined}
