...
export function AppShell({ activeView, selectedSociety, selectedProfessional, selectedClientType, selectedClientMode, settingsEntity, onRefresh, onNavigate, onNavigateSociety, onNavigateProfessional, onNavigateClientType, onNavigateSettings, children }: AppShellProps) {
  ...
  const currentLabel = activeView==='master-data'?'Definições':activeView==='admin-users'?'Utilizadores':activeView==='admin-access-logs'?'Registos de acesso':navigation.find(({ id }) => id === activeView)?.label ?? 'Carina - Legal'
  const isNavigationSelected = (id:ViewId) => activeView===id || (id==='admin'&&(activeView==='admin-users'||activeView==='admin-access-logs'||activeView==='master-data'||activeView==='imports'||activeView==='import-review'))
