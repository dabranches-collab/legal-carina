...
export function AppShell({ activeView, selectedSociety, selectedProfessional, selectedClientType, selectedClientMode, settingsEntity, onRefresh, onCreateWorkEntry, onCreateClient, onNavigate, onNavigateSociety, onNavigateProfessional, onNavigateClientType, onNavigateRetainers, onNavigateSettings, children }: AppShellProps) {
 ...
 useEffect(()=>{
  ...
  const [expandedMenu, setExpandedMenu] = useState<ViewId|null>(() => (activeView==='retainers'||activeView==='provisions')?'clients':activeView==='billing'||activeView==='professionals'||activeView==='clients' ? activeView : activeView==='admin'||activeView==='admin-users'||activeView==='master-data'||activeView==='imports'||activeView==='import-review' ? 'admin' : null)
  ...
  useEffect(()=>{
    const activeMenu=(activeView==='retainers'||activeView==='provisions')?'clients':activeView==='billing'||activeView==='professionals'||activeView==='clients'
      ? activeView
  ...
  return (
    <div className="min-h-screen bg-background text-text-primary">
      ...
      <div className={`transition-[padding] duration-200 ${collapsed ? 'lg:pl-20' : 'lg:pl-64'}`}>
        <header ref={headerRef} className="app-shell-header sticky top-0 z-20 flex flex-wrap items-center gap-3 border-b border-accent/30 bg-navigation text-navigation-text shadow-sm xl:flex-nowrap sm:gap-4">
          ...
          <nav aria-label="Localização" className="order-last min-w-0 basis-full pb-2 xl:order-none xl:flex-1 xl:basis-auto sm:py-2"><ol className="space-y-0.5">{locationLevels.map((level,index)=><li key={`${level.label}-${index}`} aria-current={index===locationLevels.length-1?'page':undefined} className={`flex min-w-0 items-center gap-2 ${index===0?'font-display text-4xl font-semibold leading-tight sm:text-[2.8125rem]':index===1?'text-[1.3125rem] font-semibold leading-tight text-accent/90':'text-lg font-medium leading-tight text-accent/75'}`}><Icon name={level.icon} className={`${index===0?'size-9':'size-[1.3125rem]'} shrink-0`}/><span className="min-w-0 break-words">{level.label}</span></li>)}<li className="truncate pl-8 text-[0.68rem] leading-tight text-accent/65">{pageDescription[activeView] ?? 'Área de trabalho da plataforma.'}</li></ol></nav>
          <div className="flex shrink-0 items-center gap-3 sm:gap-4">
          <button type="button" onClick={onCreateWorkEntry} className="flex size-20 shrink-0 flex-col items-center justify-center gap-0.5 rounded-lg border border-accent bg-accent px-2 font-semibold text-navigation shadow-sm transition hover:brightness-105" aria-label="Criar novo registo" title="Criar novo registo"><span className="text-xl leading-none" aria-hidden="true">＋</span><span className="text-center text-sm leading-tight">Novo<br/>registo</span></button>
          <button type="button" onClick={onCreateClient} className="flex size-20 shrink-0 flex-col items-center justify-center gap-0.5 rounded-lg border border-accent bg-accent px-2 font-semibold text-navigation shadow-sm transition hover:brightness-105" aria-label="Criar novo cliente" title="Criar novo cliente"><span className="text-xl leading-none" aria-hidden="true">＋</span><span className="text-center text-sm leading-tight">Novo<br/>cliente</span></button>
          </div>
