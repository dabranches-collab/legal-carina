...
interface AppShellProps { activeView: ViewId; selectedSociety:string|null; selectedProfessional:string|null; selectedClientType:'individual'|'company'|'mixed'|null; selectedClientMode:'dashboard'|'list'; settingsEntity:'clients'|'billing_entities'|null; onRefresh:()=>void; onNavigate: (view: ViewId) => void; onNavigateSociety:(name:string)=>void; onNavigateProfessional:(name:string)=>void; onNavigateClientType:(type:'individual'|'company'|'mixed',mode:'dashboard'|'list')=>void; onNavigateSettings:(target:'admin'|'clients'|'billing_entities')=>void; children: ReactNode }

export function AppShell({ activeView, selectedSociety, selectedProfessional, selectedClientType, selectedClientMode, settingsEntity, onRefresh, onNavigate, onNavigateSociety, onNavigateProfessional, onNavigateClientType, onNavigateSettings, children }: AppShellProps) {
  ...
  const [expandedMenu, setExpandedMenu] = useState<ViewId|null>(() => activeView==='billing'||activeView==='professionals'||activeView==='clients' ? activeView : activeView==='admin'||activeView==='admin-users'||activeView==='master-data'||activeView==='imports'||activeView==='import-review' ? 'admin' : null)
  ...
  useEffect(()=>{const db=supabase;if(!db)return;let active=true;void(async()=>{const [societies,professionals,profiles]=await Promise.all([db.from('billing_entities').select('name').eq('active',true),db.from('professionals').select('display_name').eq('active',true),db.from('client_profiles').select('client_id,client_type').eq('active',true)]);if(active){setBillingSocieties((societies.data??[]).map(item=>item.name).filter((name):name is string=>Boolean(name)).sort((a,b)=>a.localeCompare(b,'pt-PT')));setProfessionalNames((professionals.data??[]).map(item=>item.display_name).filter((name):name is string=>Boolean(name)).sort((a,b)=>a.localeCompare(b,'pt-PT')));const types=new Map<string,Set<string>>();for(const profile of profiles.data??[]){const current=types.get(profile.client_id)??new Set<string>();current.add(profile.client_type);types.set(profile.client_id,current)}setHasMixedClients([...types.values()].some(value=>value.size>1))}})();return()=>{active=false}},[refreshing])
  const menuLabel=(name:string)=>name===name.toLocaleUpperCase('pt-PT')?name.charAt(0).toLocaleUpperCase('pt-PT')+name.slice(1).toLocaleLowerCase('pt-PT'):name
  ...
  const isNavigationSelected = (id:ViewId) => (id==='clients'&&activeView==='clients') || (id==='admin'&&(activeView==='admin'||activeView==='admin-users'||activeView==='master-data'||activeView==='imports'||activeView==='import-review')) || (activeView===id&&!(id==='billing'&&Boolean(selectedSociety))&&!(id==='professionals'&&Boolean(selectedProfessional)))
  const subLabel = activeView==='billing'&&selectedSociety?menuLabel(selectedSociety):activeView==='professionals'&&selectedProfessional?menuLabel(selectedProfessional):activeView==='clients'&&selectedClientType?({individual:'Particulares',company:'Empresas',mixed:'Mistos'} as const)[selectedClientType]:activeView==='master-data'?(settingsEntity==='billing_entities'?'Sociedades':'Clientes'):activeView==='admin-users'?'Utilizadores':activeView==='imports'?'Importações':activeView==='import-review'?'Revisão de importações':null
      ...
      <aside aria-label="Navegação principal" className={`app-shell-sidebar fixed inset-y-0 left-0 z-40 flex flex-col border-r border-border bg-navigation text-navigation-text transition-[width,transform] duration-200 ${collapsed ? 'lg:w-20' : 'lg:w-64'} w-[min(18rem,calc(100vw-var(--safe-right)))] ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="scrollbar-thin flex-1 overflow-y-auto px-3 py-4">
          <ul className="space-y-1">
            ...
            {item.id==='billing'&&!collapsed&&expanded&&<ul className="mb-2 ml-4 mt-1 space-y-1" aria-label="Sociedades">{billingSocieties.map(name=>{const selectedSub=activeView==='billing'&&selectedSociety===name;return <li key={name}><button type="button" onClick={()=>onNavigateSociety(name)} aria-current={selectedSub?'page':undefined} className={`min-h-9 w-full rounded-md border px-2 py-1 text-left text-xs normal-case transition-colors ${selectedSub?'border-accent bg-accent font-semibold text-navigation':'border-accent/30 bg-surface/5 text-accent/80 hover:border-accent/60 hover:bg-surface/10 hover:text-accent'}`}>{menuLabel(name)}</button></li>})}</ul>}
            {item.id==='professionals'&&!collapsed&&expanded&&<ul className="mb-2 ml-4 mt-1 space-y-1" aria-label="Responsáveis">{professionalNames.map(name=>{const selectedSub=activeView==='professionals'&&selectedProfessional===name;return <li key={name}><button type="button" onClick={()=>onNavigateProfessional(name)} aria-current={selectedSub?'page':undefined} className={`min-h-9 w-full rounded-md border px-2 py-1 text-left text-xs normal-case transition-colors ${selectedSub?'border-accent bg-accent font-semibold text-navigation':'border-accent/30 bg-surface/5 text-accent/80 hover:border-accent/60 hover:bg-surface/10 hover:text-accent'}`}>{menuLabel(name)}</button></li>})}</ul>}
            {item.id==='clients'&&!collapsed&&expanded&&<ul className="mb-2 ml-4 mt-1 space-y-1" aria-label="Tipos de cliente">{([['individual','Particulares'],['company','Empresas'],...(hasMixedClients?[['mixed','Mistos'] as const]:[])] as const).map(([type,label])=>{const selectedSub=selectedClientType===type;return <li key={type}><button type="button" onClick={()=>onNavigateClientType(type,'dashboard')} aria-expanded={selectedSub} aria-current={selectedSub&&selectedClientMode==='dashboard'?'page':undefined} className={`min-h-9 w-full rounded-md border px-2 py-1 text-left text-xs transition-colors ${selectedSub&&selectedClientMode==='dashboard'?'border-accent bg-accent font-semibold text-navigation':'border-accent/30 bg-surface/5 text-accent/80 hover:border-accent/60 hover:bg-surface/10 hover:text-accent'}`}>{label}</button>{selectedSub&&<ul className="ml-3 mt-1 space-y-1" aria-label={`${label}: vistas`}><li><button type="button" onClick={()=>onNavigateClientType(type,'list')} aria-current={selectedClientMode==='list'?'page':undefined} className={`min-h-8 w-full rounded-md border px-2 py-1 text-left text-[0.7rem] ${selectedClientMode==='list'?'border-accent bg-accent/90 font-semibold text-navigation':'border-accent/25 bg-surface/5 text-accent/75 hover:border-accent/50'}`}>Lista</button></li></ul>}</li>})}</ul>}
