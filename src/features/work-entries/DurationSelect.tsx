...
export function DurationSelect({value,onChange,compact=false}:Props){
 ...
 const control=compact?'control h-8 min-w-14 px-1 text-xs':'control mt-1 w-full px-2 tabular-nums'
 return <div className={`grid ${compact?'grid-cols-3 gap-1':'grid-cols-[repeat(3,minmax(4rem,1fr))] gap-2'}`}>
