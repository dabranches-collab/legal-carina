...
export function CreateWorkEntryModal({onClose,onCreated}:{onClose:()=>void;onCreated:()=>void}){
  ...
  const [activity,setActivity]=useState(''),[hours,setHours]=useState(0),[minutes,setMinutes]=useState(0),[observations,setObservations]=useState('')
  ...
  return <div className="app-safe-fixed fixed z-[90] grid place-items-center overflow-y-auto bg-navigation/65 p-4" role="dialog" aria-modal="true" aria-labelledby="create-work-title" onMouseDown={event=>{if(event.target===event.currentTarget&&!saving)onClose()}}><form onSubmit={submit} className="card my-auto w-full max-w-3xl p-5 sm:p-6">
    ...
    {loading?<div role="status" className="mt-5 h-52 animate-pulse rounded-xl bg-surface-subtle"/>:<div className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <fieldset><legend className="text-sm font-semibold">Duração</legend><div className="mt-1 grid grid-cols-2 gap-2"><label className="text-xs text-text-secondary">Horas<input type="number" min="0" max="24" value={hours} onChange={event=>setHours(Math.max(0,Number(event.target.value)))} className="control mt-1 w-full px-3"/></label><label className="text-xs text-text-secondary">Minutos<input type="number" min="0" max="59" value={minutes} onChange={event=>setMinutes(Math.min(59,Math.max(0,Number(event.target.value))))} className="control mt-1 w-full px-3"/></label></div></fieldset>
