...
import { fixedFeeFinancialPosition } from '../clients/fixedFeeFinance'
...
export type OpenFixedFee={id:string;client_id:string;title:string;agreed_amount:number;currency:string;vat_rate:number|null;provision_applied?:number;is_invoiced:boolean;invoice_date:string|null;created_at:string}
...
async function loadOpenFixedFees():Promise<OpenFixedFee[]>{
 ...
 for(let from=0;;from+=500){const result=await supabase.from('fixed_fee_jobs').select('id,client_id,title,agreed_amount,currency,vat_rate,is_invoiced,invoice_date,created_at').neq('status','cancelled').eq('is_paid',false).order('id').range(from,from+499);if(result.error){if(result.error.code==='PGRST205'||result.error.code==='42P01'||/fixed_fee_jobs.*schema cache/i.test(result.error.message))return[];throw result.error}const page=(result.data??[]) as OpenFixedFee[];jobs.push(...page);if(page.length<500)break}
 if(!jobs.length)return jobs
 const appliedResult=await supabase.rpc('get_fixed_fee_provision_totals')
 ...
 return jobs.map(job=>({...job,provision_applied:applied.get(job.id)??0}))
}
async function mergeFixedFees(rows:Debtor[],jobs:OpenFixedFee[]):Promise<Debtor[]>{
 ...
 for(const job of jobs){if(job.currency!=='EUR')throw new Error('Existem trabalhos a preço fixo noutra moeda; os totais não podem ser somados.');const row=byId.get(job.client_id);if(!row)throw new Error('Há trabalhos a preço fixo sem ficha de cliente acessível.');const position=fixedFeeFinancialPosition({agreedAmount:Number(job.agreed_amount),isInvoiced:job.is_invoiced,isPaid:false,provisionApplied:Number(job.provision_applied??0),vatRate:job.vat_rate??undefined});if(position.toReceive<=0)continue;if(job.is_invoiced){row.fixedUnpaidCount=(row.fixedUnpaidCount??0)+1;row.fixedUnpaidAmount=(row.fixedUnpaidAmount??0)+position.toReceive}else{row.fixedPendingCount=(row.fixedPendingCount??0)+1;row.fixedPendingAmount=(row.fixedPendingAmount??0)+position.toReceive}const date=job.invoice_date??job.created_at.slice(0,10);if(!row.oldestDate||date<row.oldestDate){row.oldestDate=date;row.oldestKind=job.is_invoiced?'preço fixo':'preço fixo pendente';row.oldestInvoiceDate=job.invoice_date}}
