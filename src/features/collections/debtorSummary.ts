...
async function fetchDebtors():Promise<Debtor[]>{
 if(!supabase)throw new Error('Ligação ao Supabase indisponível.')
 const db=supabase
 const fast=await db.rpc('get_receivable_client_summary')
 if(!fast.error)return (fast.data as Debtor[]).map(row=>({...row,unpaidCount:Number(row.unpaidCount),unpaidMinutes:Number(row.unpaidMinutes),unpaidAmount:row.unpaidAmount==null?null:Number(row.unpaidAmount),uninvoicedCount:Number(row.uninvoicedCount),uninvoicedMinutes:Number(row.uninvoicedMinutes),uninvoicedAmount:row.uninvoicedAmount==null?null:Number(row.uninvoicedAmount),retainerCount:Number(row.retainerCount),retainerAmount:Number(row.retainerAmount),retainerPendingCount:Number(row.retainerPendingCount),retainerPendingAmount:Number(row.retainerPendingAmount)}))
 ...
 return buildDebtors(unpaid.items.map(withClient),uninvoiced.items.map(withClient),charges,clients)
}
