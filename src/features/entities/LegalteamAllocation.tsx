...
export function LegalteamAllocation({societyId,refreshKey=0,onSaved}:{societyId:string;refreshKey?:number;onSaved?:()=>void}){
 ...
 return <section className="card min-w-0 p-4 sm:p-6" aria-label="Repartição LEGALTEAM">
  ...
  <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_13rem]">
   ...
   <label className="text-xs font-semibold">Pagamento<select aria-label="Estado de pagamento" className="control allocation-editable mt-1 w-full px-3 text-sm" value={paidOnly?'paid':'all'} onChange={e=>{setPaidOnly(e.target.value==='paid');setSelected('all')}}><option value="all">Todos os registos</option><option value="paid">Apenas registos pagos</option></select></label>
  </div>
