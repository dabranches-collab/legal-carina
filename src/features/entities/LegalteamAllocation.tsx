...
type Attention='client'|'task'|'executor'|'price'|null
...
export function LegalteamAllocation({societyId,refreshKey=0,onSaved}:{societyId:string;refreshKey?:number;onSaved?:()=>void}){
 ...
 const resultsRef=useRef<HTMLDivElement>(null),[resultsRequest,setResultsRequest]=useState(0)
 ...
 const paymentWork=useMemo(()=>periodWork.filter(r=>payment==='all'||(payment==='paid'?r.is_paid:!r.is_paid)),[periodWork,payment])
 const clients=useMemo(()=>{
  const entries=new Map<string,ClientSummary>()
  for(const row of paymentWork){const client=entries.get(row.client_id)??{id:row.client_id,name:row.client_name,records:0,minutes:0};client.records++;client.minutes+=row.duration_minutes;entries.set(client.id,client)}
  return [...entries.values()].sort((a,b)=>a.name.localeCompare(b.name,'pt-PT'))
 },[paymentWork])
 const scope=useMemo(()=>paymentWork.filter(r=>clientIds===null||clientIds.includes(r.client_id)),[paymentWork,clientIds])
 ...
 const missingClients=useMemo(()=>{
  ...
  return [...grouped.values()].sort((a,b)=>a.name.localeCompare(b.name,'pt-PT'))
 },[gaps.client])
 const filtered=useMemo(()=>{
  const rows=attention?gaps[attention]:scope
 ...
 const choosePerson=(id:string)=>{setSelected(id);setAttention(null);showResults()}
 ...
 const clientColumns:TableColumn<ClientSummary>[]=[
  ...
  {id:'actions',label:'Consultar',value:()=>'',render:c=><div className="flex flex-wrap gap-2"><a className="control inline-flex min-h-10 items-center px-3" href={clientHref(c.id)} onClick={event=>{if(!event.ctrlKey&&!event.metaKey&&!event.shiftKey&&!event.altKey){event.preventDefault();openClientRecord(c.id)}}}>Abrir ficha</a><button type="button" className="control min-h-10 px-3" onClick={()=>{setClientIds([c.id]);setAttention(null);showResults()}}>Ver registos</button></div>,width:260},
 ]
 ...
 return <section className="card min-w-0 p-4 sm:p-6" aria-label="Repartição LEGALTEAM">
  ...
  <div className="mt-5 flex flex-wrap items-end gap-4">
   <div className="grid w-full grid-cols-2 gap-2 sm:w-auto sm:grid-cols-[9.5rem_9.5rem]">
    <label className="min-w-0 text-xs font-semibold">Data inicial<CalendarDateInput required disabled={loading} ariaLabel="Data inicial da repartição" value={start} onChange={v=>changeDate('start',v)} className="allocation-editable mt-1 w-full px-2 text-sm"/></label>
    <label className="min-w-0 text-xs font-semibold">Data final<CalendarDateInput required disabled={loading} ariaLabel="Data final da repartição" value={end} onChange={v=>changeDate('end',v)} className="allocation-editable mt-1 w-full px-2 text-sm"/></label>
   </div>
   <fieldset className="grid min-w-0 flex-1 basis-80 grid-cols-2 gap-2 sm:grid-cols-4"><legend className="sr-only">Percentagens da repartição</legend>
    {rateFields.map(([key,label])=><label key={key} className="min-w-0 text-xs font-semibold">{label}<span className="relative mt-1 block"><input aria-label={`${label} (%)`} type="number" min="0" max="100" step="0.01" value={ratesInput[key]} onChange={e=>setRatesInput(previous=>({...previous,[key]:e.target.value}))} className="control allocation-editable w-full min-w-0 pl-3 pr-8 text-base tabular-nums"/><span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-text-secondary">%</span></span></label>)}
   </fieldset>
  </div>
  ...
  <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_13rem]">
   <details className="rounded-lg border border-border bg-surface-subtle p-3"><summary aria-label="Seleccionar clientes considerados" className="cursor-pointer text-sm font-semibold"><span>Registos considerados · Clientes com movimentos no período seleccionado</span><span className="mt-2 flex items-center gap-2 rounded-lg border border-secondary bg-secondary-soft px-3 py-2"><span aria-hidden="true" className="text-success">{selectedClients.length?'✓':'—'}</span><span className="min-w-0 flex-1 truncate">{clientIds===null?'Todos':selectedClients.length?`${selectedClients[0].name}${selectedClients.length>1?` (${selectedClients.length})`:''}`:'Nenhum cliente seleccionado'}</span><span aria-hidden="true">⌄</span></span></summary>
   ...
   <label className="text-xs font-semibold">Pagamento<select aria-label="Estado de pagamento" className="control allocation-editable mt-1 w-full px-3 text-sm" value={payment} onChange={e=>{setPayment(e.target.value as typeof payment);setSelected('all')}}><option value="all">Todos os registos</option><option value="paid">Apenas registos pagos</option><option value="unpaid">Apenas registos não pagos</option></select></label>
  </div>
  ...
  {loading&&work.length===0?<p role="status" className="mt-5">A calcular a repartição…</p>:error?<div role="alert" className="mt-5 text-danger">{error}<button type="button" className="control ml-3 px-3" onClick={()=>setRefresh(n=>n+1)}>Voltar a tentar</button></div>:!validDates&&dates!==null?<p role="alert" className="mt-5 text-danger">Escolha uma data inicial igual ou anterior à data final.</p>:<>
   <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Pendências da LEGALTEAM">
    {([['client','Clientes sem angariador'],['task','Registos sem angariador da tarefa'],['executor','Registos sem responsável de execução'],['price','Ver registos sem montante']] as const).map(([key,label])=><button key={key} type="button" aria-pressed={attention===key} onClick={()=>{setAttention(attention===key?null:key);setSelected('all');showResults()}} className={`flex min-h-20 min-w-0 items-center justify-between gap-3 rounded-lg border-2 p-3 text-left ${attention===key?'border-danger bg-danger-soft text-danger ring-2 ring-danger/25':gaps[key].length?'border-danger/60 bg-danger-soft text-danger hover:border-danger':'border-border bg-surface-subtle text-text-secondary'}`}><span className="text-sm font-bold">{label}{key==='client'&&<span className="mt-1 block text-xs font-normal text-text-secondary">{missingClients.length} {missingClients.length===1?"cliente":"clientes"}</span>}</span><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${gaps[key].length?'bg-danger text-white':'bg-secondary-soft text-text-primary'}`}>{gaps[key].length} {gaps[key].length===1?"registo":"registos"}</span></button>)}
   </div>
   ...
   {map&&<>
    ...
    {map.missingPrice>0&&<div role="status" className="mt-3 flex flex-wrap items-center gap-2 text-sm text-danger"><span>{map.missingPrice} registos sem montante para repartir. As horas estão incluídas, mas falta valorizar estes serviços.</span></div>}
   ...
   <div ref={resultsRef} tabIndex={-1} role="region" aria-label="Resultados do pré-filtro" className="mt-4 rounded-lg focus:outline-none"><h3 className="mb-3 font-semibold">{attention==='client'?'Clientes sem angariador':attention==='task'?'Registos sem angariador da tarefa':attention==='executor'?'Registos sem responsável de execução':attention==='price'?'Registos sem montante':'Registos da repartição'} · {attention==='client'?missingClients.length:filtered.length}</h3>{attention==='client'?<StandardDataTable key="clients" id="legalteam-missing-client-referrer" label="Clientes sem angariador" rows={missingClients} columns={clientColumns} rowKey={c=>c.id} onRowDoubleClick={c=>openClientRecord(c.id)}/>:<StandardDataTable key={`work-${resultsRequest}`} id="legalteam-allocation" label="Registos da repartição" rows={filtered} columns={columns} rowKey={r=>r.id} onRowDoubleClick={r=>setEditing(r.id)}/>}</div>
  </>}
