...
export function saveCreditUsagePdf(account:CreditAccount,usage:CreditUsage){
 ...
 line(`Registos desde ${usage.startsOn?creditDate(usage.startsOn):'o depósito'} até ${creditDate(todayIso())}`)
 ...
 doc.save(`acompanhamento-provisao-${formatDate(new Date(),'')}.pdf`)
}
...
export function saveCreditPdf(account:CreditAccount,movements:CreditMovement[],from='',to=''){
  createCreditPdf(account,movements,from,to).save(`extracto-provisoes-${account.client_name.normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9]+/g,'-')}-${formatDate(new Date(),'')}.pdf`)
}
