...
export function ClientDocumentsPanel({firmId,clientId,readOnly=false}:{firmId:string;clientId:string;readOnly?:boolean}) {
  ...
  const columns:TableColumn<ClientDocument>[]=[
    ...
    {id:'date',label:'Data',kind:'date',value:item=>item.document_date,render:item=>item.document_date?formatDate(item.document_date):'—'},
    {id:'expiry',label:'Validade',kind:'date',value:item=>item.expires_at,render:item=>{const state=validity(item.expires_at);return <span className={`font-semibold ${state.tone}`}>{item.expires_at?`${formatDate(item.expires_at)} · `:''}{state.label}</span>}},
