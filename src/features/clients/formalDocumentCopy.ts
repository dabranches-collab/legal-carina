...
function integerWords(value:number,language:FormalLanguage):string{
 ...
 if(language==='pt'&&parts.length>1&&rest>=100&&rest%100!==0)return `${parts.slice(0,-1).join(' e ')} ${parts.at(-1)}`
