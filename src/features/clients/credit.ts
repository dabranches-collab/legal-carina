...
export type ProvisionNote = { id:string; revision?:number; number:string; issued_at:string; subtotal:number; vat_rate:number; vat:number; total:number; deducted:number; remaining:number; balance_after:number; items:ProvisionWork[]; document_options?:{client_name?:string;society_name?:string;language?:string;translation?:import('./documentTranslation').DocumentTranslation|null;expenses?:Array<{id:string;work_entry_id:string;amount:number;currency:string;observations:string|null}>} }
