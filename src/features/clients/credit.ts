...
export type ProvisionNote = { id:string; revision?:number; number:string; issued_at:string; subtotal:number; vat_rate:number; vat:number; total:number; deducted:number; remaining:number; balance_after:number; items:ProvisionWork[]; document_options?:{presentation?:import('./formalDocumentPdf').FormalSnapshot;client_name?:string;society_name?:string;language?:string;fixed_fee_paid?:boolean;fixed_fee_payment?:{provision:number;external:number};direct_payment?:{amount:number;reference?:string};translation?:import('./documentTranslation').DocumentTranslation|null;expenses_included?:boolean;expenses?:Array<{id:string;work_entry_id:string;amount:number;currency:string;observations:string|null}>;expense_attachment_ids?:string[]} }
...
export const creditDate=(date:string)=>formatDate(date)
