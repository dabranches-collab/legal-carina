...
export type FixedFeeJob = { id:string;client_id:string;billing_entity_id:string|null;title:string;agreed_amount:number;currency:string;status:string;is_invoiced:boolean;is_paid:boolean;invoice_date:string|null;created_at:string;provision_applied?:number;vat_rate?:number }
...
export function buildFixedFeeLines(jobs:FixedFeeJob[],work:FixedFeeWork[]):FixedFeeLine[]{
 ...
 for(const job of jobs){if(job.status==='cancelled')continue;if(job.currency!=='EUR')throw new Error('Há trabalhos a preço fixo noutra moeda. Os totais em euros não podem ser somados.')
  const rows=byJob.get(job.id)??[],position=fixedFeeFinancialPosition({agreedAmount:Number(job.agreed_amount),isInvoiced:job.is_invoiced,isPaid:job.is_paid,provisionApplied:Number(job.provision_applied??0),vatRate:job.vat_rate}),allocated=allocatedAmounts(position.agreed,rows),receivedAllocation=allocatedAmounts(position.received,rows)
...
export async function loadFixedFeeLines():Promise<FixedFeeLine[]>{
 ...
 for(let from=0;;from+=500){const result=await supabase.from('fixed_fee_jobs').select('id,client_id,billing_entity_id,title,agreed_amount,currency,status,is_invoiced,is_paid,invoice_date,created_at').order('id').range(from,from+499);if(result.error){if(result.error.code==='PGRST205'||result.error.code==='42P01'||/fixed_fee_jobs.*schema cache/i.test(result.error.message))return[];throw result.error}const page=(result.data??[]) as FixedFeeJob[];jobs.push(...page);if(page.length<500)break}
 if(!jobs.length)return[]
 const societyIds=[...new Set(jobs.map(job=>job.billing_entity_id).filter((id):id is string=>Boolean(id)))]
 const societies=await readIdBatches(societyIds,(ids,from,to)=>supabase!.from('billing_entities').select('id,default_vat_rate').in('id',ids).order('id').range(from,to))
 const vatRates=new Map(societies.map(row=>[row.id,Number(row.default_vat_rate)]))
 for(const job of jobs){if(job.billing_entity_id)job.vat_rate=vatRates.get(job.billing_entity_id)}
