...
export type FixedFeeJob = { id:string;client_id:string;client_name?:string;client_type?:string;mixed_client?:boolean;billing_entity_id:string|null;billing_entity_name?:string;title:string;agreed_amount:number;currency:string;status:string;is_invoiced:boolean;is_paid:boolean;invoice_date:string|null;created_at:string;provision_applied?:number;vat_rate?:number|null }
export type FixedFeeWork = { id:string;fixed_fee_job_id:string;professional_id:string;professional_name?:string;client_profile_id?:string;client_type?:string;work_date:string;duration_minutes:number;archive_status?:string|null }
export type FixedFeeLine = { jobId:string;entryId:string|null;title:string;clientId:string;clientName?:string|null;clientType:string|null;mixedClient:boolean;billingEntityId:string|null;billingEntityName:string|null;professionalId:string|null;professionalName:string|null;archiveStatus?:string|null;date:string;minutes:number;amount:number;isInvoiced:boolean;invoiced:number;paid:number;unpaid:number;uninvoiced:number }
...
export async function loadFixedFeeLines(filter?:string|{billingEntityId?:string;clientId?:string}):Promise<FixedFeeLine[]>{
 ...
 const [work,appliedResult]=await Promise.all([
  readIdBatches(jobs.map(job=>job.id),(ids,from,to)=>supabase!.from('work_entries').select('id,fixed_fee_job_id,professional_id,client_profile_id,work_date,duration_minutes,archive_status').in('fixed_fee_job_id',ids).order('id').range(from,to)),
 ...
 const [billing,people,clients,profiles]=await Promise.all([
  ...
  readIdBatches([...new Set(jobs.map(job=>job.client_id))],(ids,from,to)=>supabase!.from('clients').select('id,client_type,display_name').in('id',ids).order('id').range(from,to)),
 ...
 const defaultTypes=new Map(clients.map(row=>[row.id,row.client_type])),clientNames=new Map(clients.map(row=>[row.id,row.display_name])),profileTypes=new Map(profiles.map(row=>[row.id,row.client_type])),mixedClients=new Set<string>()
