...
export type CreditUsage = {startsOn:string|null;subtotal:number;vat:number;total:number;consumed:number;balance:number;excess:number;minutes:number;missingPrice:number;invoicedUnpaid:number;rows:Array<CreditWork&{balance:number}>}
...
export function calculateCreditUsage(detail:CreditDetail,work:CreditWork[],vatRate:number,today:string):CreditUsage{
 ...
 const eligible=work.filter(row=>startsOn&&row.work_date>=startsOn&&row.work_date<=today&&row.client_id===account.client_id&&row.billing_entity_id===account.billing_entity_id&&row.currency===account.currency&&row.billing_scope==='standard'&&row.is_billable&&!row.is_paid&&!noted.has(row.id)&&!['cancelled','uncollectible_uninvoiced','uncollectible_invoiced'].includes(row.status)).sort((a,b)=>a.work_date.localeCompare(b.work_date)||a.id.localeCompare(b.id))
 let cents=0
 ...
 return {startsOn,subtotal:cents/100,vat:vatCents/100,total:totalCents/100,consumed:(Math.round(Number(account.consumed)*100)+Math.min(ledgerCents,totalCents))/100,balance:Math.max(0,ledgerCents-totalCents)/100,excess:Math.max(0,totalCents-ledgerCents)/100,minutes:rows.reduce((n,row)=>n+row.duration_minutes,0),missingPrice:rows.filter(row=>row.effective_amount===null).length,invoicedUnpaid:rows.filter(row=>row.is_invoiced).length,rows}
}
