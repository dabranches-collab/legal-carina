import { creditDate, creditMoney, type CreditAccount } from './credit'
import type { CreditUsage } from './creditUsage'
import { saveCreditUsagePdf } from './creditPdf'

export function CreditUsageSummary({account,usage}:{account:CreditAccount;usage:CreditUsage}){
 return <section aria-label="Consumo da provisão nos registos" className="space-y-3 rounded-xl border border-border bg-surface-subtle p-4">
  ...
  <button type="button" className="min-h-11 rounded-lg border border-border bg-surface px-3 text-sm font-semibold" onClick={()=>saveCreditUsagePdf(account,usage)}>Guardar mapa de consumo PDF</button>
 </section>
}
