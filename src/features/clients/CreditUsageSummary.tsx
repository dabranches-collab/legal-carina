...
export function CreditUsageSummary({account,usage,movements=[],onSaved}:{account:CreditAccount;usage:CreditUsage;movements?:CreditMovement[];onSaved?:()=>void}){
 ...
 const noted=movements.filter(movement=>movement.kind==='consumption'&&!movement.reversed&&movement.note).flatMap(movement=>movement.note!.items.map(item=>({...item,noteNumber:movement.note!.number})))
 return <section aria-label="Consumo da provisão nos registos" className="space-y-3 rounded-xl border border-border bg-surface-subtle p-4">
  ...
  <details><summary className="min-h-11 cursor-pointer py-2 font-semibold">Ver os registos considerados na provisão ({noted.length+usage.rows.length})</summary><div className="space-y-2">{noted.map(row=><article key={`${row.noteNumber}-${row.id}`} className="rounded-lg border border-border bg-surface p-3 text-sm"><strong>{creditDate(row.work_date)} · {row.duration_minutes} min · {creditMoney(row.effective_amount,account.currency)}</strong><p className="mt-1 break-words">{row.activity_description}</p><p className="mt-1 text-xs text-text-secondary">Já incluído na nota {row.noteNumber}; a provisão aplicada consta do histórico. A nota não comprova pagamento.</p></article>)}{usage.rows.map(row=><article key={row.id} tabIndex={0} onDoubleClick={()=>setEditing(row.id)} onKeyDown={event=>{if(event.key==='Enter'&&event.target===event.currentTarget){event.preventDefault();setEditing(row.id)}}} className="rounded-lg border border-border bg-surface p-3 text-sm"><strong>{creditDate(row.work_date)} · {row.duration_minutes} min · {row.effective_amount===null?'Sem preço':creditMoney(row.effective_amount,account.currency)}</strong><p className="mt-1 break-words">{row.activity_description}</p><p className="mt-1 text-xs text-text-secondary">Ainda sem nota de provisão.</p></article>)}{!noted.length&&!usage.rows.length&&<p className="text-sm text-text-secondary">Não existem registos valorizados nesta conta.</p>}</div></details>
