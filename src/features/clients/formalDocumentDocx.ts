...
export async function createFormalDocumentDocx(snapshot:FormalSnapshot,rows:Entry[],expenses:EntryExpense[],note:ProvisionNote|null,isCollection=false){
 ...
 const direct=Number(note?.document_options?.direct_payment?.amount??0),excess=note?Math.max(0,Math.round((direct+Number(note.deducted)-Number(note.total))*100)/100):0
 if(direct>0)children.push(para(`${language==='pt'?'Pagamento directo recebido':language==='en'?'Direct payment received':'Paiement direct reçu'}: ${money(direct)}`,true))
 if(excess>0)children.push(para(`${language==='pt'?'Excedente a regularizar fora das provisões':language==='en'?'Excess to settle outside advances':'Excédent à régulariser hors provisions'}: ${money(excess)}`,true))
