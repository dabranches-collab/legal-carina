...
export async function createFormalDocumentDocx(snapshot:FormalSnapshot,rows:Entry[],expenses:EntryExpense[],note:ProvisionNote|null,isCollection=false){
 ...
 if(note){children.push(para(`${language==='pt'?'Nota':language==='en'?'Note':'Note'}: ${note.number}${note.revision?` · v${note.revision}`:''}`,true));if(hasProvision)children.push(para(`${language==='pt'?'Provisão descontada':language==='en'?'Advance deducted':'Provision déduite'}: ${money(note.deducted)}`,true));children.push(para(`${language==='pt'?'Valor a pagar':language==='en'?'Amount due':'Montant à payer'}: ${money(note.remaining)}`,true));if(hasProvision)children.push(para(`${language==='pt'?'Saldo de provisão após esta nota':language==='en'?'Advance balance after this note':'Solde de provision après cette note'}: ${money(note.balance_after)}`,true))}
