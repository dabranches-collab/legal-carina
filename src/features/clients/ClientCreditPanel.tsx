...
import { saveCreditPdf, saveProvisionNotePdf } from './creditPdf'
...
export function ClientCreditPanel({clientId,readOnly=false,onRequestEdit}:{clientId:string;readOnly?:boolean;onRequestEdit?:()=>void}){
  ...
  const [noteOpen,setNoteOpen]=useState(false),[from,setFrom]=useState(''),[to,setTo]=useState('')
  ...
  async function choose(id:string){const version=++generation.current;setAccountId(id);setDetail(null);setLoading(true);setError('');try{const result=await supabase!.rpc('get_client_credit_detail',{p_account_id:id});if(result.error)throw result.error;if(version===generation.current)setDetail(result.data as CreditDetail)}catch(cause){if(version===generation.current)setError(message(cause))}finally{if(version===generation.current)setLoading(false)}}
  ...
  const statement=creditStatement(detail?.movements??[],from,to),invalidPeriod=Boolean(from&&to&&from>to)
  async function payment(){await mutate(async()=>{
    ...
    paymentRequest.current=crypto.randomUUID();setAmount('');setReference('');setPaymentOpen(false);await refresh();setNotice('Provisão registada. O valor será descontado ao emitir a Nota de Honorários.')
  })}
  async function reverse(){await mutate(async()=>{const result=await supabase!.rpc('reverse_client_credit',{p_movement_id:reverseId,p_reason:reason,p_request_id:reverseRequest.current});if(result.error)throw result.error;reverseRequest.current=crypto.randomUUID();setReverseId('');setReason('');await refresh(accountId);setNotice('Estorno registado. O movimento original mantém-se no histórico.')})}
  return <section aria-label="Provisões para honorários" className="min-w-0 space-y-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-semibold">Provisões para honorários</h3><p className="mt-1 text-sm text-text-secondary">Saldo inicial, reforços e descontos nas Notas de Honorários.</p></div>{readOnly?<button type="button" className={button} onClick={onRequestEdit}>Gerir provisões</button>:<button type="button" className={`${button} bg-primary text-surface`} disabled={busy||loading} onClick={()=>setPaymentOpen(!paymentOpen)}>Registar provisão</button>}</div>
    ...
    {paymentOpen&&!readOnly&&<fieldset disabled={busy} className="grid gap-3 rounded-xl border border-border bg-surface-subtle p-4 sm:grid-cols-2"><legend className="px-2 font-semibold">Saldo inicial / reforço da provisão</legend>
      ...
      <label>Data de entrada<input className="control mt-1 block w-full min-w-0 px-2" type="date" max={today()} value={date} onChange={event=>setDate(event.target.value)}/></label>
    ...
    {loading?<p role="status">A carregar o provisões…</p>:!detail&&!error?<p className="rounded-lg bg-surface-subtle p-4">Ainda não existem pagamentos antecipados neste cliente.</p>:null}
    {detail&&!loading&&<>
      <div className="grid gap-3 sm:grid-cols-3">{[['Recebido (líquido de estornos)',detail.account.received],['Provisão descontada',detail.account.consumed],['Saldo disponível',detail.account.balance]].map(([label,value])=><div key={label} className="rounded-xl border border-border bg-surface-subtle p-4"><p className="text-sm text-text-secondary">{label}</p><p className="financial-value mt-1 text-xl font-semibold">{creditMoney(Number(value),detail.account.currency)}</p></div>)}</div>
      {!readOnly&&<button type="button" className={`${button} bg-primary text-surface`} disabled={busy||Number(detail.account.balance)<=0} onClick={()=>setNoteOpen(true)}>Emitir Nota de Honorários com provisão</button>}
      <div className="space-y-3"><div className="flex flex-wrap items-end gap-3"><h4 className="mr-auto font-semibold">Extracto de movimentos</h4><label className="text-xs">Lançamentos desde<input className="control mt-1 block min-w-0 px-2" type="date" value={from} onChange={event=>setFrom(event.target.value)}/></label><label className="text-xs">Até<input className="control mt-1 block min-w-0 px-2" type="date" value={to} onChange={event=>setTo(event.target.value)}/></label><button type="button" className={button} disabled={invalidPeriod||busy} onClick={()=>{try{saveCreditPdf(detail.account,detail.movements,from,to)}catch(cause){setError(message(cause))}}}>Guardar extracto PDF</button></div>
        {invalidPeriod?<p role="alert" className="text-danger">A data final deve ser igual ou posterior à inicial.</p>:<><p className="text-sm">Saldo inicial: <strong>{creditMoney(statement.opening,detail.account.currency)}</strong></p><div className="space-y-2">{statement.rows.map(row=><article key={row.id} className="rounded-lg border border-border p-3"><div className="flex flex-wrap justify-between gap-2"><strong>{creditDate(row.recorded_at)} · {creditKind(row.kind)}{row.reversed?' · Estornado':''}</strong><span className="financial-value font-semibold">{creditMoney(row.amount,detail.account.currency)}</span></div><p className="mt-1 break-words text-sm">{row.reference}</p>{row.note&&<><p className="mt-1 text-sm text-text-secondary">Total da nota: {creditMoney(row.note.total,detail.account.currency)} · Provisão descontada: {creditMoney(row.note.deducted,detail.account.currency)} · A pagar: {creditMoney(row.note.remaining,detail.account.currency)}</p><details className="mt-2 text-sm"><summary className="cursor-pointer font-semibold">Ver os {row.note.items.length} registos da nota</summary>{row.note.items.map(item=><p key={item.id} className="mt-2 break-words">{creditDate(item.work_date)} · {item.activity_description} · {item.duration_minutes} min · {creditMoney(item.effective_amount,detail.account.currency)}</p>)}</details><button type="button" className={`${button} mt-2`} onClick={()=>saveProvisionNotePdf(detail.account,row.note!,row.reversed||row.kind==='reversal')}>Guardar cópia da nota</button></>}<div className="mt-2 flex flex-wrap items-center justify-between gap-2"><span className="text-sm">Saldo: {creditMoney(row.balance,detail.account.currency)}</span>{!readOnly&&!row.reversed&&row.kind!=='reversal'&&<button type="button" className={button} disabled={busy} onClick={()=>{setReverseId(row.id);setReason('');reverseRequest.current=crypto.randomUUID()}}>Estornar</button>}</div>{reverseId===row.id&&<div className="mt-2 space-y-2"><label className="block text-sm">Motivo do estorno<input className="control mt-1 w-full px-2" maxLength={1000} value={reason} onChange={event=>setReason(event.target.value)}/></label><button type="button" className={`${button} text-danger`} disabled={busy||reason.trim().length<3} onClick={()=>void reverse()}>Confirmar estorno</button><button type="button" className={`${button} ml-2`} disabled={busy} onClick={()=>setReverseId('')}>Cancelar</button></div>}</article>)}</div><p className="text-sm font-semibold">Saldo final: {creditMoney(statement.closing,detail.account.currency)}</p></>}
      </div>
    </>}
    {noteOpen&&detail&&<HonorariumNoteModal clientId={clientId} clientName={detail.account.client_name} initialProvisionAccountId={accountId} onClose={()=>{setNoteOpen(false);void mutate(()=>refresh(accountId))}}/>}
  </section>
}
