...
type AdminUser = { userId:string; username:string; displayName:string; display_name?:string; role:Role|'owner'; active:boolean; invitedAt:string; lastSignInAt:string|null }
...
export function AdminPage() {
  ...
  async function saveAccess(event:FormEvent) {
    ...
    try {
      ...
      else if (editing.userId===currentUser?.id) {
      ...
      if(editing.userId!==currentUser?.id) await load()
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_24rem]">
      <div className="card overflow-hidden"><div className="flex items-center justify-between border-b border-border p-5"><div><h2 className="font-semibold">Utilizadores da aplicação</h2><p className="mt-1 text-sm text-text-secondary">Perfis e visibilidade são aplicados no backend.</p></div><button onClick={() => void load()} className="control grid size-10 place-items-center" aria-label="Actualizar utilizadores"><Icon name="trend" className="size-4"/></button></div>
        {loading ? <div role="status" className="space-y-2 p-5">{Array.from({length:3},(_,i)=><div key={i} className="h-12 animate-pulse rounded bg-surface-subtle"/>)}</div> : <div className="overflow-x-auto"><table className="w-full min-w-[800px] text-left text-sm"><thead className="bg-surface-subtle text-text-secondary"><tr><th className="px-5 py-3">Nome</th><th className="px-5 py-3">Utilizador</th><th className="px-5 py-3">Perfil</th><th className="px-5 py-3">Estado</th><th className="px-5 py-3">Último acesso</th><th className="px-5 py-3">Acções</th></tr></thead><tbody>{users.map(user=><tr key={user.userId} className="border-t border-border"><td className="px-5 py-3 font-medium">{user.displayName || 'Sem nome'}</td><td className="px-5 py-3 text-text-secondary">{user.username || 'PIN por configurar'}</td><td className="px-5 py-3">{roleLabels[user.role]}</td><td className="px-5 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${user.active?'bg-success-soft text-success':'bg-surface-subtle text-text-secondary'}`}>{user.active?'Activo':'Inactivo'}</span></td><td className="px-5 py-3 text-text-secondary">{user.lastSignInAt?new Date(user.lastSignInAt).toLocaleString('pt-PT'):'Ainda não entrou'}</td><td className="px-5 py-3"><button onClick={() => void openPermissions(user)} className="rounded-lg border border-border px-3 py-2 font-semibold text-primary">Configurar</button></td></tr>)}</tbody></table></div>}
      </div>
    ...
    {isOwner && <section className="card overflow-hidden" aria-labelledby="login-log-title"><div className="border-b border-border p-5"><h2 id="login-log-title" className="font-semibold">Histórico de acessos</h2><p className="mt-1 text-sm text-text-secondary">Visível apenas para o proprietário. Entradas consecutivas do mesmo utilizador aparecem agrupadas.</p></div>{loginError?<p role="alert" className="p-5 text-sm text-danger">{loginError}</p>:loginGroups.length===0?<p className="p-5 text-sm text-text-secondary">Ainda não existem entradas registadas.</p>:<div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-surface-subtle text-text-secondary"><tr><th className="px-5 py-3">Utilizador</th><th className="px-5 py-3">Primeira entrada do bloco</th><th className="px-5 py-3">Última entrada do bloco</th><th className="px-5 py-3 text-right">Entradas</th><th className="px-5 py-3">Detalhe</th></tr></thead><tbody>{loginGroups.map((group,index)=><tr key={`${group.userId}-${group.lastAt}-${index}`} className="border-t border-border"><td className="px-5 py-3"><span className="block font-medium">{group.displayName}</span><span className="text-xs text-text-secondary">{group.username}</span></td><td className="px-5 py-3 tabular-nums">{new Date(group.firstAt).toLocaleString('pt-PT')}</td><td className="px-5 py-3 tabular-nums">{new Date(group.lastAt).toLocaleString('pt-PT')}</td><td className="px-5 py-3 text-right font-semibold tabular-nums">{group.count}</td><td className="px-5 py-3">{group.count>1?<details><summary className="cursor-pointer font-semibold text-primary">Ver entradas</summary><ul className="mt-2 space-y-1 text-xs text-text-secondary">{group.events.map((date,eventIndex)=><li key={`${date}-${eventIndex}`}>{new Date(date).toLocaleString('pt-PT')}</li>)}</ul></details>:<span className="text-text-secondary">Entrada única</span>}</td></tr>)}</tbody></table></div>}</section>}
