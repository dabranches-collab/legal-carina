...
export function AdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    try { const data = await invoke({ action:'list_users', firmId:membership.firm_id }); setUsers((data.users ?? []).map((item:AdminUser)=>({...item,displayName:(item.userId===currentUser?.id?currentUser.user_metadata?.display_name:'')||item.displayName||item.display_name||item.username}))) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível listar os utilizadores.') }
    setLoading(false)
  }, [invoke,currentUser])
  ...
  async function createUser(event:FormEvent) {
    ...
    try { const temporaryPin=pin; await invoke({ action:'create_pin_user', firmId, displayName, username, pin, role }); setAccessMessage(makeAccessMessage(username,temporaryPin)); setNotice(`Acesso criado para ${displayName}.`); setDisplayName(''); setUsername(''); setPin(''); await load() }
  ...
  async function copyAccessMessage() {
    ...
    catch { setError('Não foi possível copiar automaticamente. Selecione o texto da mensagem e copie-o manualmente.') }
  }
