  ...
  return (
    <section aria-labelledby="import-title">
      ...
      {analysis && summary && <div className="mt-8 space-y-8">
        ...
        <div className="card overflow-x-auto"><table className="min-w-full text-left text-xs"><caption className="p-4 text-left font-display text-lg font-semibold">Pré-visualização local (primeiras 8 linhas)</caption><thead className="bg-surface-subtle"><tr>{analysis.preview[0].map((header, index) => <th key={`${header}-${index}`} className="whitespace-nowrap px-3 py-2">{header}</th>)}</tr></thead><tbody>{analysis.preview.slice(1).map((row, rowIndex) => <tr key={rowIndex} tabIndex={0} aria-selected={selectedPreviewRow===rowIndex} onClick={()=>setSelectedPreviewRow(rowIndex)} onFocus={()=>setSelectedPreviewRow(rowIndex)} className={`h-[2.125rem] border-t border-border odd:bg-surface-subtle even:bg-surface hover:bg-secondary-soft ${selectedPreviewRow===rowIndex?'table-row-active outline outline-1 outline-secondary':''}`}>{row.map((value, index) => <td key={index} className="max-w-56 truncate px-3 py-1">{value}</td>)}</tr>)}</tbody></table></div>
