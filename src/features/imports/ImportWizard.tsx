...
export function ImportWizard() {
  ...
  const previewColumns=useResizablePlainTable('import-preview',Array.from({length:analysis?.preview[0]?.length??0},()=>180))
    ...
    <section aria-labelledby="import-title">
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
        <div onDragOver={(event) => event.preventDefault()} onDrop={dropped} className="card rounded-2xl border-2 border-dashed border-accent/50 p-8 text-center">
          ...
          <button type="button" onClick={() => inputRef.current?.click()} className="mt-5 min-h-9 rounded-lg bg-primary px-4 py-1.5 text-sm font-semibold text-surface hover:bg-primary-hover">Seleccionar ficheiro</button>
      ...
      {analysis && summary && <div className="mt-8 space-y-8">
        ...
        <div className="card overflow-x-auto"><table style={{width:previewColumns.width}} className="table-fixed text-left text-xs">{previewColumns.colgroup}<caption className="p-4 text-left font-display text-lg font-semibold">Pré-visualização local (primeiras 8 linhas)</caption><thead className="bg-surface-subtle"><tr>{analysis.preview[0].map((header, index) => previewColumns.header(index,header,'truncate px-3 py-2'))}</tr></thead><tbody>{analysis.preview.slice(1).map((row, rowIndex) => <tr key={rowIndex} tabIndex={0} aria-selected={selectedPreviewRow===rowIndex} onClick={()=>setSelectedPreviewRow(rowIndex)} onFocus={()=>setSelectedPreviewRow(rowIndex)} className={`h-[2.125rem] border-t border-border odd:bg-surface-subtle even:bg-surface hover:bg-secondary-soft ${selectedPreviewRow===rowIndex?'table-row-active outline outline-1 outline-secondary':''}`}>{row.map((value, index) => <td key={index} className="truncate px-3 py-1">{value}</td>)}</tr>)}</tbody></table></div>
        ...
        {importResult&&<p role="status" className="rounded-xl border border-success/30 bg-success-soft p-4 text-success">Reconciliação concluída: {importResult.newRows} novos, {importResult.updatedRows} actualizados, {importResult.unchangedRows} sem alterações e {importResult.invalidRows} inválidos. {importResult.missingRows} movimentos anteriores ficaram apenas sinalizados como ausentes. Lote: <code>{importResult.importId}</code>.</p>}
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-primary p-6 text-surface"><label className="flex items-start gap-3 text-sm"><input type="checkbox" disabled={!remoteValidated} checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} className="mt-1" /><span>{remoteValidated?'Confirmei as linhas novas, inalteradas, alteradas, ausentes e o impacto financeiro. Pretendo reconciliar este lote no Supabase.':summary.conflictRows>0?'A importação está bloqueada porque existem alterações manuais em conflito.':'A importação permanece bloqueada até concluir a comparação segura com todos os movimentos existentes.'}</span></label><div className="flex gap-2"><button type="button" onClick={cancel} className="min-h-8 rounded-md border border-surface/40 px-3 py-1 text-sm">Cancelar</button><button type="button" disabled={busy} onClick={() => file && void run(file, analysis.selectedSheet, analysis.mapping)} className="min-h-8 rounded-md bg-surface px-3 py-1 text-sm font-semibold text-primary disabled:opacity-50">Validar</button><button type="button" disabled={!confirmed||!remoteValidated||busy||Boolean(importResult)} onClick={()=>void commit()} className="min-h-8 rounded-md bg-accent px-3 py-1 text-sm font-semibold text-primary disabled:cursor-not-allowed disabled:opacity-50">{busy?'A reconciliar…':'Reconciliar'}</button></div></div>
      </div>}
    </section>
  )
}
