...
export function LoginPage({ busy, error, notice, onLogin, onRecover, onEmailLink, onVerifyEmailCode, onPasskeyLogin, onClearError }: LoginPageProps) {
  ...
  return <main className="app-safe-screen grid bg-background lg:grid-cols-[1.05fr_.95fr]">
    ...
    <section className="flex items-center justify-center p-6 sm:p-10"><div className="w-full max-w-md"><div className="mb-8 lg:hidden"><div className="grid size-11 place-items-center rounded-xl bg-primary font-display text-surface">LC</div></div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-secondary">Legal Carina</p><h1 className="mt-2 font-display text-3xl font-semibold">{recovery ? 'Recuperar password' : 'Iniciar sessão'}</h1><p className="mt-3 text-sm leading-6 text-text-secondary">{recovery ? 'Enviaremos um link de recuperação se o endereço estiver autorizado.' : 'Introduza as credenciais fornecidas pela administração.'}</p>
      ...
      </form>}{!emailCodeMode && !recovery && <div className="mt-4 grid gap-3"><div className="rounded-lg border border-border bg-surface-subtle p-3"><p className="text-xs uppercase tracking-wider text-text-secondary">Utilizador</p><p className="mt-1 font-semibold">dabranches</p></div><button type="button" disabled={busy} onClick={() => void onPasskeyLogin()} className="w-full rounded-lg border border-border bg-surface px-4 py-3 font-semibold text-primary transition-colors hover:bg-surface-muted disabled:opacity-50">Entrar com PIN</button><button type="button" disabled={busy || !email} onClick={async () => { onClearError(); if (await onEmailLink(email)) setEmailCodeMode(true) }} className="text-sm font-semibold text-secondary hover:underline disabled:opacity-50">Receber código temporário por email</button><button type="button" disabled={busy || !email} onClick={() => { onClearError(); setEmailCodeMode(true) }} className="text-sm font-semibold text-secondary hover:underline disabled:opacity-50">Já tenho um código</button></div>}{!emailCodeMode && <button type="button" onClick={() => { onClearError(); setRecovery((value) => !value) }} className="mt-5 text-sm font-semibold text-secondary hover:underline">{recovery ? 'Voltar ao login' : 'Preciso de recuperar o acesso'}</button>}<p className="mt-8 border-t border-border pt-5 text-xs leading-5 text-text-secondary">Não existe registo público. Contacte a administração para obter acesso.</p>
    </div></section>
  </main>
}
