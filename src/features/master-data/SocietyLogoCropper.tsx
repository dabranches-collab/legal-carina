...
export function SocietyLogoCropper({disabled,existingUrl,onChange,onRemove}:{disabled:boolean;existingUrl:string;onChange:(blob:Blob)=>void;onRemove:()=>void}){
  ...
  useEffect(()=>{if(!source){imageRef.current=null;return}const image=new Image();image.crossOrigin='anonymous';image.onload=()=>{imageRef.current=image;setPosition({x:0,y:0});setZoom(1);draw()};image.onerror=()=>setError('Não foi possível apresentar o logótipo guardado.');image.src=source;return()=>{if(source.startsWith('blob:'))URL.revokeObjectURL(source)}},[source])
  function draw(nextPosition=position,nextZoom=zoom){const canvas=canvasRef.current,image=imageRef.current;if(!canvas||!image)return;const context=canvas.getContext('2d');if(!context)return;const base=Math.max(canvas.width/image.naturalWidth,canvas.height/image.naturalHeight),scale=base*nextZoom,width=image.naturalWidth*scale,height=image.naturalHeight*scale,x=(canvas.width-width)/2+nextPosition.x,y=(canvas.height-height)/2+nextPosition.y;context.clearRect(0,0,canvas.width,canvas.height);context.drawImage(image,x,y,width,height)}
  ...
  function apply(){const preview=canvasRef.current;if(!preview)return;const output=document.createElement('canvas');output.width=OUTPUT_WIDTH;output.height=OUTPUT_HEIGHT;const context=output.getContext('2d');if(!context)return;context.drawImage(preview,0,0,OUTPUT_WIDTH,OUTPUT_HEIGHT);output.toBlob(blob=>{if(blob)onChange(blob)},'image/png',.92)}
