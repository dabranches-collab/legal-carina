...
import { cropSourceRect, logoFitScale, squareCropAroundBounds, squareCropInside, type CropInsets } from './logoCropMath'

GlobalWorkerOptions.workerSrc=pdfWorkerUrl

const PREVIEW_WIDTH=600,PREVIEW_HEIGHT=600,MAX_OUTPUT_SIDE=1200
...
async function fullImageBlob(source:string){
  const image=await new Promise<HTMLImageElement>((resolve,reject)=>{const value=new Image();value.onload=()=>resolve(value);value.onerror=()=>reject(new Error('Não foi possível preparar o logótipo.'));value.src=source})
  const sourceSide=Math.min(image.naturalWidth,image.naturalHeight),scale=Math.min(1,MAX_OUTPUT_SIDE/sourceSide),canvas=document.createElement('canvas')
  canvas.width=Math.max(1,Math.round(sourceSide*scale));canvas.height=canvas.width
  const context=canvas.getContext('2d');if(!context)throw new Error('Não foi possível preparar o logótipo.')
  ...
  return await new Promise<Blob>((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(blob):reject(new Error('Não foi possível preparar o logótipo.')),'image/png',.92))
}
...
export function SocietyLogoCropper({disabled,existingUrl,onChange,onRemove}:{disabled:boolean;existingUrl:string;onChange:(blob:Blob)=>void;onRemove:()=>void}){
  ...
  async function choose(file?:File){if(!file)return;setError('');try{const nextSource=await sourceFromFile(file);setSource(nextSource);onChange(await fullImageBlob(nextSource))}catch(cause){setError(cause instanceof Error?cause.message:'Não foi possível ler o ficheiro.')}}
  function drop(event:DragEvent){event.preventDefault();if(!disabled)void choose(event.dataTransfer.files[0])}
  function update(side:keyof CropInsets,value:number){const opposite=side==='left'?'right':side==='right'?'left':side==='top'?'bottom':'top';setCrop(current=>{const next={...current,[side]:Math.max(0,Math.min(value,94-current[opposite]))};return imageRef.current?squareCropInside(imageRef.current.naturalWidth,imageRef.current.naturalHeight,next):next})}
  ...
  function trimWhiteMargins(){
    ...
    try{
      ...
      setCrop(squareCropAroundBounds(canvas.width,canvas.height,{left:minX,right:maxX,top:minY,bottom:maxY}));setError('')
