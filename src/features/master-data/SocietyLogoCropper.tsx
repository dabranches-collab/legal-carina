...
import { cropAroundBounds, cropSourceRect, logoFitScale, type CropInsets } from './logoCropMath'

GlobalWorkerOptions.workerSrc=pdfWorkerUrl

const PREVIEW_WIDTH=600,PREVIEW_HEIGHT=600,MAX_OUTPUT_SIDE=2400
...
async function fullImageBlob(source:string){
  ...
  const scale=Math.min(1,MAX_OUTPUT_SIDE/Math.max(image.naturalWidth,image.naturalHeight)),canvas=document.createElement('canvas')
...
export function SocietyLogoCropper({disabled,existingUrl,onChange,onRemove}:{disabled:boolean;existingUrl:string;onChange:(blob:Blob)=>void;onRemove:()=>void}){
  ...
  function trimWhiteMargins(){
    ...
    try{
      ...
      setCrop(cropAroundBounds(canvas.width,canvas.height,{left:minX,right:maxX,top:minY,bottom:maxY}));setError('')
