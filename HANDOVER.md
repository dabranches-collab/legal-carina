...
- A migration local `20260924190000_include_expenses_in_honorarium_totals.sql` adapta a função de gravação para validar as despesas activas na base e guardar o mesmo total do documento. Foi comparada com o histórico remoto, cujo último registo era `20260922153227_reapply_new_credit_on_honorarium_revision`. A migration **não foi aplicada**; antes de emissão real com despesas é necessário testá-la em staging, executar os gates de publicação e promovê-la antes do frontend.
