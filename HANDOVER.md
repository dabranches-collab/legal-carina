...
- Sem migration, alteração de dados de clientes ou escrita remota no Supabase. Testes sintéticos: 50 ficheiros/267 testes unitários aprovados antes do último ajuste de formatos, build e dry-run Cloudflare aprovados. Validação final E2E e repetição da suite completa em curso; completar antes da publicação.
...
- Worker `legal-carina` publicado em `https://legal-carina.dabranches.workers.dev`: versão 0.12.7, deployment `08e1ef5c-380d-4725-b7fd-96df2e5512b7`, Version ID `ee689cb4-9c11-4a15-ac22-2e662c4b148e`, 100% desde `2026-09-24 19:01:06 UTC`. `/release-notes.json` respondeu 0.12.7. Rollback frontend: Version ID `5102698c-cd33-4111-a6ce-19780b0dc318` (0.12.6).
- Migration `20260924185918_include_expenses_in_honorarium_totals` aplicada isoladamente no Supabase `vtvvqyebigflgqccbqsw`; a função instalada passou a somar as despesas validadas após o IVA dos honorários. As 13 versões existentes conservaram exactamente os totais, descontos e saldos agregados. A auditoria de continuidade registou 5 utilizadores, 5 pertenças, 7 concessões, 6 permissões financeiras, zero órfãos e 48/48 tabelas públicas com RLS. Advisors sem novos findings críticos.
- Backup físico da base confirmado no painel Supabase em `2026-09-24 05:44:16 UTC`; PITR inactivo. A branch temporária `qa-honorarium-0127-20260924` falhou ao reconstruir migrations antigas e foi eliminada, com apenas `main` na lista posterior. O ensaio da migration em staging/pgTAP não ficou concluído. O backup da base não contém os 26 objectos privados do Storage; uma cópia independente desses objectos não foi comprovada. A alteração aplicada não modifica dados existentes nem Storage; rollback da função pela definição anterior da migration `20260922153227`.
...
- A data por extenso usa o mês com inicial maiúscula. Os parágrafos e descrições dos registos são justificados. A nota passa a somar as despesas activas dos registos escolhidos, depois do IVA dos honorários, sem aplicar novo IVA ao valor introduzido como despesa. A tabela de despesas conserva a largura da tabela de trabalho e mostra apenas Montante e Observações. O parágrafo inicial e a linha Total discriminam honorários, IVA, despesas e total por extenso. As notas anteriores conservam o total financeiro gravado e identificam as respectivas despesas como informativas.
- Novo botão «Pré-visualizar sem guardar» cria PDF e Word de rascunho assinalados como sem emissão. Não chama a gravação, não consome numeração e não escreve na base. Foi usado apenas um documento sintético de 6 600,00 € + 1 518,00 € de IVA + 399,00 € de despesas = 8 517,00 €, revisto visualmente em PDF. Nenhuma nota real foi apagada ou refeita.
- A migration local, depois registada como `20260924185918_include_expenses_in_honorarium_totals.sql`, adapta a função de gravação para validar as despesas activas na base e guardar o mesmo total do documento. Neste ensaio ainda não tinha sido aplicada; a publicação e as limitações do ensaio remoto constam da entrada mais recente acima.
- Validação local: segurança de ficheiros, lint, TypeScript, 49 ficheiros/254 testes unitários e build aprovados; 47 testes dirigidos e o teste sintético do fluxo de pré-visualização no Chromium passaram. A repetição do E2E em servidor externo isolado encerrou normalmente com `1 passed (7.5s)`. A suite E2E integral, staging da migration, CI e dry-run Cloudflare continuam pendentes para eventual publicação. Pré-visualização local aberta e servida em `http://127.0.0.1:5178/?view=clients`.

## 24-09-2026 — 0.12.7 local: sugestão de códigos por lacuna

- A criação de clientes sugere agora o menor número livre nas séries de Particulares (`02`) e Empresas (`01`). A consulta considera códigos existentes em `clients` e `client_profiles`, incluindo vertentes inactivas e fichas sem perfil, para evitar sugestões de códigos ocupados. O operador continua a poder editar o código antes de guardar; a restrição única da base continua a impedir duplicados se duas pessoas criarem fichas ao mesmo tempo.
- Validação: lint, TypeScript, compilação e dois testes dirigidos passaram. No browser integrado, a criação local apresentou os primeiros códigos livres nas duas vertentes; a ficha de teste foi fechada sem guardar. Produção mantém a versão 0.12.6 e a regra anterior até publicação expressa da 0.12.7.

## 24-09-2026 — correcção operacional de fichas na base de produção

- Alteração de dados, sem alteração de código, migration ou publicação do frontend. Numa transacção auditada, foi fundida uma ficha particular duplicada na ficha principal: três registos de trabalho foram transferidos, mantendo IDs, duração, preço e estados. A grafia do nome principal foi conciliada com a ficha removida. A ficha principal ficou com quatro registos, 150 minutos e 375,00 €; o código duplicado ficou livre nas fichas e perfis activos.
- Outra ficha já tinha a vertente particular activa, mas conservava um código com prefixo empresarial. O código da ficha e do perfil foi corrigido para o próximo código particular livre; os 62 registos de trabalho mantiveram os mesmos IDs e ligações. Verificação por junção das tabelas: 62/62 registos resolvem agora o código e tipo particulares, nenhum resolve o antigo código empresarial. O código libertado podia ser introduzido manualmente; a sugestão automática é revista no lote local acima.
...
- Diagnóstico apenas de leitura do caso comunicado: na primeira consulta, três registos não pagos tinham valor/hora e montante nulos, o que explica o erro genérico ao incluí-los. Numa segunda consulta após a validação, os 20 registos não pagos já tinham ambos os valores preenchidos; a alteração dos dados ocorreu fora deste lote. O browser integrado abriu a Nota de Honorários com o filtro «Não pagos», 20 registos seleccionáveis e sem alerta, coerente com o estado actualizado. Não foram alterados valores ou estados da cliente neste lote.
- Validação local concluída: segurança de ficheiros, lint, TypeScript, 48 ficheiros/250 testes unitários e build aprovados. Dois E2E sintéticos da Nota de Honorários passaram na pré-visualização compilada, incluindo aviso imediato, filtro, exclusão do registo sem preço, emissão, histórico e revisão. O servidor Vite de desenvolvimento não concluiu o carregamento nos testes iniciais; a repetição no servidor de pré-visualização passou. A pré-visualização real 0.12.7 responde em `http://127.0.0.1:5173/` e foi verificada no browser integrado. A suite E2E integral, CI e dry-run Cloudflare ficam pendentes antes de eventual publicação.
...
- Supabase de produção `vtvvqyebigflgqccbqsw`: migration isolada `20260922153227_reapply_new_credit_on_honorarium_revision`, confirmada no histórico e nas definições das duas funções alteradas. A revisão aplica novo crédito mesmo quando os registos não mudam, aceita opções de documento até 60 kB em ambas as funções e subtrai pagamentos directos ao valor por pagar sem os lançar como provisão. Histórico de migrations lido antes e depois, sem `db push` global ou `migration repair`.
- Cópia física diária da base confirmada no painel Supabase em `2026-09-22 05:50:33 UTC`, com opção de restauro. PITR não está activo. A cópia independente de 13 objectos privados do Storage em R2 foi confirmada em 18-09-2026 no handover anterior; a cópia diária da base não contém os objectos.
- Revisão real solicitada pelo titular foi gravada através da aplicação local ligada ao mesmo backend, preservando o histórico de estorno, a versão anterior e o pagamento da primeira factura com IVA. Verificação de leitura posterior confirmou a versão actual com saldo por pagar e saldo de provisões ambos zero, pagamento directo separado, excedente explícito por regularizar e nenhuma provisão adicional criada. Os registos de trabalho não foram marcados como facturados ou pagos pela nota, de acordo com o comportamento existente; a confirmação fiscal dos documentos respectivos continua a ser uma operação própria.
- Validações locais: segurança de ficheiros, lint, tipos, runtime de tradução, build, 46 ficheiros/242 testes unitários e dry-run Cloudflare aprovados. E2E dirigido da revisão da nota passou em preview com dados sintéticos. Os quatro cenários responsivos do formulário precisaram de actualizar os selectores para os novos rótulos de IVA; a repetição dirigida percorreu os passos mas o runner Playwright local ficou pendente no encerramento, como ocorrera em lotes anteriores. A CI deverá confirmar o conjunto completo antes da publicação. Pré-visualização PDF da nota revista abriu duas páginas na versão local; a revisão reutilizou a tradução histórica sem depender do serviço externo.
...
- Branch `codex/desktop-layout-0.12.3`, criada sobre `main`/`origin/main` `f96c762cde8c7e00ed278cf295a3d5c4c6603338` após confirmar checkout limpo e executar `git fetch --all --prune`. A produção permanece na 0.12.2; `/release-notes.json` online confirmou a versão. Esta versão 0.12.3 está apenas em preparação local, sem deployment, Version ID ou alterações no Supabase.
- Revistos os 19 menus com dados sintéticos em desktop 1920×1240 e 1920×1080, com zoom de 100% e 150%, mais dois cenários em modo escuro. Os painéis de entrada de Clientes, Sociedades e Responsáveis passam a usar a altura útil do ecrã quando cabem. A Visão Geral distribui os indicadores em linhas completas e evita caixas vazias esticadas. Em Registos, a barra de pré-filtros liberta espaço nas janelas com altura útil inferior a 900 píxeis.
- Nas tabelas, o espaço do cabeçalho fixo é reservado imediatamente durante o scroll, sem render intermédio. A primeira linha da lista de Utilizadores deixa de ficar sob o cabeçalho. Capturas e verificações geométricas ficam nos testes `e2e/desktop-layout.spec.ts` e `e2e/table-sticky.spec.ts`. Confirmar o resultado final dos testes e o estado da branch antes de qualquer integração ou publicação.
- Validação visual: 6 testes de matriz (19 menus nas quatro combinações pedidas e duas combinações escuras) passaram; 23 testes de cabeçalho/filtros fixos e três testes de Utilizadores passaram. No acompanhamento, 36 cenários passaram na primeira execução e o único cenário desalinhado passou após a correcção. A primeira tentativa da suite unitária sob carga concorrente foi inconclusiva; a repetição isolada passou por completo, conforme registado acima.
- O Vite de desenvolvimento ficou preso a responder à primeira página. A compilação local é servida por `vite preview` em `http://127.0.0.1:5173/`, com o mesmo proxy de autenticação das funções Supabase. HTTP 200 confirmado na rota principal e resposta HTTP 401 esperada no GET sem chave de `/supabase-functions/v1/pin-auth`, comprovando o encaminhamento. O browser integrado abriu o ecrã de entrada local identificado como 0.12.3; não foi introduzido PIN real.
...
- A criação de trabalhos a preço fixo falhava porque o comando de gravação estava dentro do formulário principal da ficha e não executava a operação própria. O painel passou a gravar explicitamente e deixou de simular gravações ou associações em memória quando o esquema remoto não está disponível. Os trabalhos persistidos aparecem na lista e permitem associar e desassociar registos.
- As RPCs de associação guardam o estado financeiro anterior e repõem-no integralmente na desassociação, incluindo tratamento, valor/hora, montante e indicadores de facturação. A distribuição ao cêntimo usa uma ordem canónica dos identificadores. Os resumos de clientes e Registos deixaram de duplicar linhas e integram o preço fixo nos filtros, totais e listas de atenção.
- Pesquisa, ficha aberta, separador e filtro de registos ficam reflectidos no URL e sobrevivem ao refresh; fechar regressa à lista e pesquisa anteriores. O modo oculto apresenta `*****` nas colunas monetárias sem revelar valores no título. O estado visível/oculto continua controlado pelo utilizador. A tabela de utilizadores já não cria filtros de coluna sobre a lista.
- Migrations aplicadas isoladamente no Supabase remoto e confirmadas no histórico: `20260921182446_preserve_fixed_fee_pricing`, `20260921182804_restore_legacy_fixed_fee_pricing` e `20260921183113_allow_audited_fixed_fee_restore`. Ensaio sintético real: trabalho `TESTE TÉCNICO PREÇO FIXO - APAGAR`, dois registos com 30 e 15 minutos, preço de 100 €, total de 45 minutos e média de 133,33 €/h. A lista global mostrou 2 registos, 45 minutos e 100 €; ao retirar ambos, os valores originais de 72,50 € e 36,25 € a 145 €/h foram repostos. O trabalho foi apagado e a consulta final confirmou zero trabalhos sintéticos.
- Validação concluída: `pnpm security:files`, `pnpm lint`, `pnpm typecheck`, 44 ficheiros/233 testes unitários, `pnpm build` e dry-run Wrangler aprovados. Na suite E2E, 119 cenários passaram inicialmente, três foram omitidos condicionalmente e três expectativas antigas sobre URL falharam; depois de as alinhar com a persistência agora exigida, os três passaram. Resultado consolidado: 122 aprovados e três omissões condicionais. Produção deve receber a 0.12.1 apenas a partir do commit centralizado no GitHub; depois do deploy, registar aqui commit, PR, CI, deployment e version ID.
- A primeira CI do PR #53 revelou que Vite 8 recusava em modo `serve` a importação directa de `public/release-notes.json` feita pelo aviso PWA. O build e o preview de produção não eram afectados. As notas passaram a ser fornecidas ao código por um módulo virtual que lê o mesmo ficheiro canónico. O endpoint de desenvolvimento respondeu 200, TypeScript e os quatro testes do aviso passaram após a correcção; a CI será repetida no novo commit.
...
- Actualização de 18-09-2026, após nova ordem «publica»: a migration local passou a incluir aplicações individuais de provisões a trabalhos, operação atómica com pagamento ou a partir do saldo, estorno controlado, RLS e consultas de totais. A ficha de Provisões pergunta claramente a que trabalhos aplicar cada montante com IVA; os resumos de entidades e «Por receber» descontam a parcela recebida sem reduzir os honorários acordados antes de IVA. A ficha de preço fixo mostra a provisão aplicada. O ensaio pgTAP sintético tem 19 verificações, incluindo saldo, abatimento e estorno, mas **ainda não pôde ser executado numa base com esquema actual**.
- Validação local do novo lote: `pnpm security:files`, lint, typecheck, 43 ficheiros/228 testes e build aprovados; dry-run Cloudflare aprovado. A suite E2E sintética percorreu 125 cenários: 118 passaram, quatro falharam no mesmo mock desactualizado da operação de provisões e três foram ignorados; após corrigir o mock, os oito testes de provisões passaram, incluindo os quatro tamanhos de ecrã. A CI do PR #44 passou em 18-09-2026 (validate, dependency-audit e scan); o Supabase Preview foi ignorado pela integração. Uma segunda branch temporária Supabase `codex-fixed-fee-012-final-check` falhou a reprodução do histórico na oitava migration, tal como a primeira; não recebeu SQL novo e foi apagada de imediato. Confirmado por `list_branches` que só resta `main`. Produção e histórico remoto não foram alterados. A repartição LEGALTEAM e a emissão de documentos continuam sem integrar os trabalhos a preço fixo. O gate de migration ensaiada em staging e estas integrações financeiras permanecem pendentes; **não publicar ainda**.
...
- Integração local em 18-09-2026: os dashboards por cliente/sociedade/responsável, gráficos, páginas de entrada e resumos por categoria adicionam analiticamente o preço fixo, conservando as horas dos registos. Em clientes mistos, a parcela segue o perfil de cada registo. Testes sintéticos focados passaram, mas a reconciliação com SQL real, provisões, documentos e LEGALTEAM continua por fazer. Não considerar os cálculos globais completos nem publicar esta versão como funcionalidade financeira concluída.
- Decisão do utilizador: ao receber uma provisão, perguntar explicitamente se quer abater trabalhos a preço fixo já existentes; escolha caso a caso, com montante manual por trabalho e opção «Não aplicar agora». O preço acordado e o valor médio/hora mantêm-se; o remanescente por receber e o saldo disponível da provisão mudam apenas após aplicação confirmada. Texto completo e casos de reconciliação em `docs/fixed-fee-reconciliation.md`. Ainda falta implementar livro de afectações, transacção, UI e estorno; nenhuma escolha automática ou redução fictícia foi introduzida.
- Fase operacional do trabalho na ficha: começa «Por iniciar», passa automaticamente a «Em curso» com o primeiro registo associado e só é marcada «Terminado» pelo operador. Retirar todos os registos não repõe «Por iniciar». A lista mostra as fases e permite filtrar. Continua independente de facturação e pagamento. A sessão autenticada do browser local expirou antes da verificação visual actualizada.
...
- Produção Cloudflare `https://legal-carina.dabranches.workers.dev` CONFIRMADA na versão 0.10.21 a 100% desde `2026-09-16 12:39:16 UTC`: deployment `dbfc95fa-7524-4d3e-bbdf-1af3ac753b47`, version `13705d2c-925f-4848-a3a0-ff1b599b8d4b`. Rollback imediato do frontend: version `bb3d809a-e94b-45ca-be7c-a8e3f6546dd3` (0.10.20). URL e notas de versão responderam HTTP 200; o browser integrado autenticado confirmou versão e vista em tabela. Código local e GitHub `main` coincidem no commit publicado.
...
- Clientes, sociedades e responsáveis têm separadores de registos dentro do diálogo. A navegação não altera o URL; fechar repõe a lista de origem. E2E sintético específico para os três tipos aprovado, bem como 208 testes unitários, 10 E2E Windows/ficha e 2 E2E da matriz iPhone em claro/escuro; a CI executou a suite integral. Segurança de ficheiros, lint, tipos, runtime do Worker e build aprovados. O browser integrado confirmou a tabela por defeito, a ficha sem espera pelos totais, a lista filtrada dentro do diálogo e o regresso ao mesmo URL. O deploy alterou apenas o frontend Worker: nenhuma migration ou escrita em dados, Auth, RLS, Storage, Edge Functions, utilizadores, permissões ou segredos.
...
- O pré-filtro «Pagos s/ factura ou data» e a respectiva contagem só incluem movimentos pagos de facturação normal sem data da factura no movimento e sem data nem número de factura associada. O marcador de excepção histórica não prevalece sobre estes dados. A migration de substituição de duas funções de leitura, sem DML nem mudança de RLS, foi aplicada isoladamente e registada remotamente como `20260916104110_filter_paid_work_without_invoice_evidence`; o ficheiro local foi alinhado ao carimbo remoto neste handover. GHH: 58 falsos positivos com data antes, zero após; contagem global 117 antes, 41 após. Não se alteraram movimentos, Auth, utilizadores, permissões ou segredos.
- As molduras gerais dos cartões e das caixas da lista de clientes têm 2 px e mais contraste. Nas tabelas partilhadas, as opções de filtro de coluna sem resultados perante os restantes filtros ficam sombreadas e indisponíveis; teste de regressão incluído. A aplicação online autenticada mostra «Versão 0.10.20» e o pré-filtro com 41; o `release-notes.json` online contém as três novidades da versão e o histórico acumulado.
- Gates locais: `pnpm security:files`, `pnpm lint`, `pnpm typecheck`, 38 ficheiros/208 testes unitários e `pnpm build` aprovados; 38 E2E focados passaram. A suite integral teve 119 aprovações, três condicionais ignorados e duas falhas intermitentes (repartição e iPhone SE), ambas aprovadas na repetição isolada. Dry-run Cloudflare com Wrangler 4.131.2 aprovado. Backup físico Supabase mais recente confirmado em `2026-09-16 05:49:52 UTC`, sem PITR; Storage não é incluído nesse backup. Linha de base antes da promoção: 5 Auth, 5 pertenças, 7 concessões, 6 permissões financeiras, zero órfãos; 46/46 tabelas públicas com RLS.
- Verificação no browser integrado autenticado: versão local 0.10.20, moldura efectiva de 2 px nas caixas de clientes, cores contrastantes em tema claro e escuro; tema claro reposto. Após fixar a espessura em 2 px, lint, tipos, os 38 E2E iPhone/tabelas e o dry-run Cloudflare passaram novamente. A CLI Supabase 2.117.0 estava disponível temporariamente, mas `migration list --linked` indicou projecto não ligado neste checkout; o histórico remoto foi listado pela integração Supabase antes e depois da aplicação. A linha de base pós-migration continuou igual (5 Auth, 5 pertenças, 7 concessões, 6 permissões financeiras, zero órfãos, 46/46 tabelas públicas com RLS). Advisors pós-migration sem erros novos; permanecem avisos anteriores de funções `security definer` e configuração Auth/RLS.

## Release 0.10.19 publicada — 16-09-2026

- Publicação da 0.10.19 autorizada pelo utilizador. PR #33 fundido no GitHub `main` em `d0dbb9224056ed16f5b223d2bedbad82e4929ba9`; branch de desenvolvimento `codex/client-summary-cards-0.10.19`, último commit de código `7afa6bd37ba1c5e50e466686be687a8fc1ee2320`. Checkout canónico `C:\Projetos\legal-carina`; branch documental pós-deploy `codex/handover-0.10.19`. Produção `https://legal-carina.dabranches.workers.dev` confirmada 0.10.19 a 100% desde `2026-09-16 09:10:55 UTC`: deployment `a12a0d63-5f1a-4a8c-9835-a57c51019b8b`, version `e0a0442b-c268-4e3d-a00c-0f759708ff19`. Rollback frontend anterior: version `4f24cadf-9d5a-4844-9ee5-be5c29346550` (0.10.18).
- Antes do deploy, corrigiu-se a sobreposição da barra de pesquisa fixa à navegação móvel e o encaixe dos controlos a 320 px. A regressão abrange os 11 modelos de iPhone da matriz local em claro e escuro. A suite local terminou com 121 E2E aprovados/3 condicionais ignorados, 38 ficheiros/207 testes unitários, segurança de ficheiros, lint, tipos, build e dry-run Cloudflare aprovados; CI do PR `35077174048`, auditoria de dependências e secret scan verdes. HTTP 200 online e seis alterações acumuladas em `release-notes.json` confirmados.
- Nenhuma migration ou escrita em dados de produção, Auth, RLS, Storage, Edge Functions, utilizadores, permissões ou segredos. O deploy preservou as variáveis Cloudflare existentes.
...
- As listas de clientes Particulares, Empresas e Mistos abrem em caixas compactas com pesquisa e alternância para tabela. A barra de pesquisa, apresentação e criação é fixa numa linha; as acções Ficha, Nota e Cobrança preservam os nomes, e só as descrições ao passar o rato explicam a ficha e a existência ou ausência de movimentos facturados por cobrar. Cobrança activa a vermelho, indisponível a verde suave.
...
- Publicação explicitamente autorizada. PR #31 fundido no `main` canónico em `7f674534631843bc52caf64fc625f8dd573bd108`; CI `35027521290`, auditoria de dependências e secret scan `35027521277` verdes. Produção CONFIRMADA a 100% desde `2026-09-15 21:54:12 UTC`: deployment `36fb5678-68a5-486f-8140-cd801c68d6a3`, version `3741a2f2-2736-4b40-ad58-6c2cc1674d38`. HTTP 200, `release-notes.json` 0.10.17 e browser integrado autenticado com versão 0.10.17 confirmados. Rollback frontend: `6930409c-b1fc-4424-9794-691e8665bd17` (0.10.16).
...
- A ficha de cliente ganhou tratamento formal (`Exmo. Senhor,`, `Exma. Senhora,`, `Exmos. Senhores,`, `Exmas. Senhoras,`). Se estiver omisso, a janela de emissão exige a escolha antes de activar a geração.
- Após autorização explícita para publicar tudo, a migration aditiva `20260915162917_add_client_honorarium_salutation.sql` foi aplicada isoladamente por `db query`, sem `db push` nem `migration repair`. Coluna, restrição e zero valores inválidos confirmados. A divergência histórica de carimbos mantém-se para reconciliação dedicada; Auth, RLS, Storage, Edge Functions, permissões, utilizadores e segredos não foram alterados. Backup físico mais recente observado: `2026-09-15 05:47:54 UTC`; advisor pós-migration sem erros.
- Verificação sintética visual concluída com renderização real pelo Microsoft Word e rasterização do PDF: uma página em ambos, sem cortes ou sobreposições, com cabeçalho/rodapé e sem texto de provisões. Gates finais aprovados: segurança de ficheiros, lint, TypeScript, 37 ficheiros/198 testes unitários, runtime do Worker, build, dry-run Cloudflare e E2E integral com 119 aprovações e três testes condicionais ignorados.
- Produção CONFIRMADA a 100% em `https://legal-carina.dabranches.workers.dev`: versão 0.10.16, deployment `66b33873-f219-4367-afe8-07bb3d47c722`, version `6930409c-b1fc-4424-9794-691e8665bd17`, desde `2026-09-15 18:23:05 UTC` (`19:23:05` em Luanda). HTTP 200 e browser autenticado confirmaram a aplicação actualizada; rollback frontend: `d0c49de5-1b5e-4b86-8cb8-6e5005627115` (0.10.15).
...
- PR #24 fundido no commit `e6ff464d782afae03912ca78e5ce2826ab0bb695`; CI `34876591087` e secret scan `34876591095` verdes. Gates locais: ficheiros sensíveis, lint, TypeScript, 35 ficheiros/191 testes unitários, runtime do Worker, build, dry-run, 119 E2E gerais e três E2E PWA dedicados.
- Cloudflare CONFIRMADO a 100% em `https://legal-carina.dabranches.workers.dev`: versão 0.10.14, deployment `e528b0a7-88b0-4b51-83b5-41d9e55fb82b`, version `455d288f-3bf0-4e63-956a-136ba2c0a203`, desde `17:51:47 UTC`. Página, manifesto e service worker responderam 200; browser autenticado confirmou a versão.
- HAPPY AMBICION foi alterada em produção: deixou de ter a vertente Particular activa e passou a Empresa com o código `01.0109`. A lista de Empresas devolve exactamente uma ficha com esse nome/código e a contagem de Particulares desceu uma unidade.
- O ensaio documental sintético revelou a causa backend: `private.audit_business_change()` recusava os metadados porque a escrita privilegiada da Edge Function não tinha `auth.uid()`, apesar de `uploaded_by` preservar o utilizador autenticado. PR #25 (`de762bf`) acrescentou etapas de erro seguras e logs internos; PR #26 (`4b33d74`) passou a aceitar `uploaded_by` como actor explícito da auditoria. As CI `34878730168` e `34880538698` e os secret scans passaram; a primeira CI exigiu uma repetição por uma falha E2E instável e alheia, aprovada na repetição.
- A Edge Function `client-documents` versão 3 está `ACTIVE`, com JWT obrigatório. Aplicou-se apenas o SQL revisto `20260914192000_allow_uploaded_by_audit_actor.sql` através de `db query`; não se executou `db push`, nem se alteraram Auth, utilizadores, permissões, buckets ou segredos. A divergência histórica de carimbos de migrations permanece bloqueada e esta aplicação directa deve ser reconciliada apenas pelo protocolo próprio.
- Teste pós-correcção CONFIRMADO no cliente sintético XPANDALPHABET: o PDF sintético foi carregado, protegido, listado como Activo e ficou com a acção Consultar. O ficheiro real `Consulta de Certidão Permanente Key Homes.pdf` não existe actualmente no computador; por isso não foi possível carregá-lo na ficha da SUSANA HENRIQUES e nenhum documento real foi fabricado ou substituído.
...
- CONFIRMADO no portal Azure: a subscrição `Azure subscription 1` foi convertida com sucesso para `Plano do Azure` (pay as you go) e está `Activo`. O ecrã pós-actualização confirma facturação acumulada de 0,00 EUR e preservação do crédito de avaliação até 07-10-2026.
- CONFIRMADO: o recurso `carina-legal-translator` permanece `Activo`, em North Europe, API `TextTranslation`, escalão de preço `Gratuito` (F0). A conversão da subscrição não alterou o escalão do Translator.
- CONFIRMADO: métrica Azure dos últimos 30 dias mostra 180,94 mil caracteres traduzidos e 0 erros totais; a quota F0 publicada é de 2 milhões de caracteres por mês. O consumo observado corresponde a cerca de 9% da quota mensal.
- CONFIRMADO: chamada real ao Azure com dois textos exclusivamente sintéticos passou em inglês e francês, preservando referências, horas, ordinais e montantes. No Worker `legal-carina`, `AZURE_TRANSLATOR_KEY` e `AZURE_TRANSLATOR_REGION` continuam configurados como segredos Cloudflare; os valores não foram lidos nem expostos.
- A continuidade deixou de depender do fim da avaliação em 07-10-2026. Podem existir cobranças apenas se forem usados serviços/quantidades fora dos montantes gratuitos; manter o Translator em F0 e vigiar a quota mensal. Nenhum código, segredo, deployment ou recurso foi alterado nesta verificação.
...
- GitHub CONFIRMADO: código funcional `274469a6bc8d1d8372d4b4983a1bbe0def8b4e00` centralizado na mesma branch e PR #14 aberto; HEAD remoto igual ao local após o push. A publicação continua dependente de «publica», configuração segura do segredo no Worker, quota da API, ensaio real e gates finais/CI verde. Não confundir os testes com fornecedor simulado com validação real da tradução.
