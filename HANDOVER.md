...
- Validação: 6 testes de matriz visual (19 menus nas quatro combinações pedidas e duas combinações escuras) passaram; 23 testes de cabeçalho/filtros fixos passaram; três testes de Utilizadores passaram. No acompanhamento, 36 cenários passaram na primeira execução e o único cenário desalinhado passou após a correcção. Os 18 testes unitários da tabela passaram após actualizar a expectativa da linha de reserva. Segurança de ficheiros, lint, TypeScript e build passaram. A suite unitária completa não terminou neste lote: uma primeira execução sob carga concorrente deu três falhas por tempo/expectativa, e uma repetição ficou sem progresso visível e foi interrompida. Não considerar esse gate concluído para publicação.
