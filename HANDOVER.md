...
## Candidato local 0.10.20 — não publicado — 16-09-2026

- Checkout canónico `C:\Projetos\legal-carina`, branch `codex/fix-paid-invoice-attention-0.10.20` a partir de `origin/main` (`bb58d3edf338593ce1ede615dc44c715af29fd65`). A versão local e as notas acumuladas estão em 0.10.20. GitHub `main` e produção continuam em 0.10.19; produção `https://legal-carina.dabranches.workers.dev`, deployment `a12a0d63-5f1a-4a8c-9835-a57c51019b8b`, version `e0a0442b-c268-4e3d-a00c-0f759708ff19`, desde `2026-09-16 09:10:55 UTC`. A ordem explícita «publica» foi recebida; promoção ainda pendente de CI, migration e deploy.
- O pré-filtro «Pagos s/ factura ou data» e a respectiva contagem têm agora uma migration proposta: só devem incluir movimentos pagos de facturação normal sem data da factura no movimento e sem data nem número de factura associada. O marcador de excepção histórica deixa de prevalecer sobre esses elementos. A verificação remota foi apenas de leitura: GHH tem 58 falsos positivos com data preenchida; a simulação da nova regra dá zero para GHH e reduz a contagem global de 117 para 41. A migration **não foi aplicada**; não houve alteração de dados, Auth, RLS, utilizadores, permissões ou segredos.
- As molduras gerais dos cartões e das caixas da lista de clientes têm mais contraste. Nas tabelas partilhadas, as opções de filtro de coluna sem resultados perante os restantes filtros ficam sombreadas e indisponíveis; teste de regressão incluído. Antes de qualquer aplicação remota da migration, reconciliar o histórico com `docs/database/migration-reconciliation.md` e confirmar a lista ligada de migrations. O CLI `supabase migration list --linked` não está instalado neste computador; a lista remota foi consultada via integração de leitura.
...
- Verificação no browser integrado autenticado: versão local 0.10.20, moldura efectiva de 2 px nas caixas de clientes, cores contrastantes em tema claro e escuro; tema claro reposto. Após fixar a espessura em 2 px, lint, tipos e os 38 E2E iPhone/tabelas passaram novamente. A CLI Supabase 2.117.0 está disponível temporariamente, mas `migration list --linked` indicou projecto não ligado neste checkout; o histórico remoto foi listado pela integração Supabase sem escrita nem ligação local.
