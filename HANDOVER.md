...
- Lacuna confirmada em 18-09-2026: os dashboards por cliente/sociedade/responsável, gráficos e resumos por categoria ainda usam os agregados antigos de registos. Os trabalhos a preço fixo só estão incluídos no acompanhamento «Por receber» e na Visão Geral preparada nesta branch. Completar e reconciliar estes painéis antes de considerar os cálculos globais correctos.
...
- Modelo puro `fixedFeeFinancialPosition` adicionado: separa preço acordado, valor a facturar, valor recebido, saldo por receber e montante recebido antes da factura. A repartição analítica por responsável usa o preço integral para a média/hora e permite provisão parcial sem duplicar receita. Sete testes focados passaram. Ainda não existe persistência nem leitura de afectações reais; a interpretação de IVA foi pedida ao utilizador antes de ligar o livro contabilístico.
