...
test('7235 registos recuperam de falha de rede com consultas curtas e despesas completas',async({page})=>{
 ...
 await expect(page.getByRole('progressbar',{name:'Progresso do carregamento da tabela'})).toBeVisible()
 await expect(page.getByRole('status').filter({hasText:'7235 registos de 7235'})).toBeVisible({timeout:20_000})
