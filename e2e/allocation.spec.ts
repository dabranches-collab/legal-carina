...
test('lista inferior abre o próprio registo e actualiza a repartição sem perder filtros',async({page})=>{
 await page.goto(`${demo}&view=billing&society=LEGALTEAM`)
 const map=page.getByRole('region',{name:'Repartição LEGALTEAM',exact:true})
 const finalDate=map.getByLabel('Data final da repartição')
 await expect(finalDate).toHaveValue('03-09-2026')
 ...
 await expect(map.getByLabel('Data final da repartição')).toHaveValue('03-09-2026')
 ...
 let openedUrl=new URL(page.url())
 expect(openedUrl.searchParams.get('record')).toBe('00000000-0000-4000-8000-000000000021')
 expect(openedUrl.searchParams.get('recordSection')).toBe('clients')
...
test('datas automáticas, percentagens editáveis e selecção múltipla recalculam imediatamente',async({page})=>{
 await page.goto(`${demo}&view=billing&society=LEGALTEAM`)
 const map=page.getByRole('region',{name:'Repartição LEGALTEAM',exact:true})
 await expect(page.getByLabel('Data inicial da repartição')).toHaveValue('01-09-2026')
 await expect(page.getByLabel('Data final da repartição')).toHaveValue('03-09-2026')
 ...
 await page.getByLabel('Data final da repartição').fill('02-09-2026')
 await expect(map.getByRole('button',{name:/Total do período/})).toContainText('1300,00')
...
test('pré-filtros contam registos sem preço e distinguem clientes de tarefas e execução',async({page})=>{
 ...
 await page.goto('/?qa-iphone=1&qa-role=admin&view=billing&society=LEGALTEAM')
 const map=page.getByRole('region',{name:'Repartição LEGALTEAM',exact:true})
 await expect(page.getByLabel('Data final da repartição')).toHaveValue('04-09-2026')
 ...
 await page.getByLabel('Data final da repartição').fill('02-09-2026');await expect(executor).toContainText('0 registos');await expect(task).toContainText('0 registos')
...
test('repartição por período, pagos, campos e responsáveis completos',async({page})=>{
 await page.goto(`${demo}&view=billing&society=LEGALTEAM`)
 const map=page.getByRole('region',{name:'Repartição LEGALTEAM',exact:true})
 await page.getByLabel('Data final da repartição').fill('30-09-2026')
 await expect(map.getByRole('button',{name:/Total do período/})).toContainText('2000,00')
