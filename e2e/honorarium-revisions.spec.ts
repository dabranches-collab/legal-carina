...
test('histórico, filtros, revisão, estorno e reemissão da nota',async({page})=>{
 ...
 const oldCopy=page.waitForEvent('download');await dialog.getByRole('button',{name:'Ver PDF v1'}).click();await (await oldCopy).saveAs('.tmp/reprint-legacy.pdf')
 ...
 const copy=page.waitForEvent('download');await dialog.getByRole('button',{name:'Guardar novamente em PDF'}).click();await (await copy).saveAs('.tmp/reprint-pt-copy.pdf')
