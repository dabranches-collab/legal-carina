...
test('carrega o login protegido sem registo público', async ({ page }) => {
  await page.goto('/')
  await expect(page).toHaveTitle(/Carina - Legal/)
  ...
  await expect(page.getByLabel('PIN de 4 algarismos')).toBeVisible()
...
test('abre a recuperação de password', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1 })).toContainText('Recuperar acesso')
  await expect(page.getByLabel('PIN de 4 algarismos')).toHaveCount(0)
})
