...
test('arrastar uma divisória altera só a largura dessa coluna e persiste', async ({page}) => {
  await page.setViewportSize({width:1440,height:800})
  await page.goto('/?qa-iphone=1&view=work')
  const region=page.getByRole('region',{name:'Registos de trabalho'})
  await expect(region.getByText('180 registos de 180')).toBeVisible()
  const table=region.getByRole('table')
  const before=await table.locator('colgroup col').evaluateAll(columns=>columns.slice(0,3).map(column=>column.getBoundingClientRect().width))
  const leftBefore=await table.boundingBox()
  const handle=region.getByRole('separator',{name:'Ajustar largura de Data',exact:true})
  ...
  await page.mouse.up()
  const after=await table.locator('colgroup col').evaluateAll(columns=>columns.slice(0,3).map(column=>column.getBoundingClientRect().width))
  const leftAfter=await table.boundingBox()
  expect(after[0]).toBeGreaterThan(before[0]+70)
  expect(after[1]).toBeCloseTo(before[1],0)
  expect(after[2]).toBeCloseTo(before[2],0)
  expect(leftAfter!.x).toBeCloseTo(leftBefore!.x,0)
  await page.reload()
  await expect(region.getByText('180 registos de 180')).toBeVisible()
  const persisted=await table.locator('colgroup col').first().evaluate(column=>column.getBoundingClientRect().width)
...
test('sticky compacto não ocupa o ecrã num iPhone e filtros abrem dentro do viewport', async ({ page }) => {
  ...
  expect(metrics.top).toBeGreaterThanOrEqual(47+8)
  expect(metrics.bottom).toBeLessThanOrEqual(metrics.height-34-8)
  await page.screenshot({ path: 'test-results/table-sticky-iphone-dark.png', fullPage: false })
  await page.keyboard.press('Escape')
  await table.getByRole('button', { name: /Colunas ·/ }).click()
  const columnsBox = await page.getByRole('dialog', { name: /Colunas visíveis/ }).boundingBox()
  expect(columnsBox).not.toBeNull()
  expect(columnsBox!.y).toBeGreaterThanOrEqual(47+8)
  expect(columnsBox!.y+columnsBox!.height).toBeLessThanOrEqual(844-34-8)
  await page.keyboard.press('Escape')
  await page.getByRole('button', { name: 'Criar novo registo' }).click()
  const modal = page.getByRole('dialog', { name: 'Criar movimento' })
  const formBox = await modal.locator('form').boundingBox()
  expect(formBox).not.toBeNull()
  expect(formBox!.y).toBeGreaterThanOrEqual(47)
  expect(formBox!.y+formBox!.height).toBeLessThanOrEqual(844-34)
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
})

test('iPhone em landscape mantém filtros, menu e cabeçalho fora das áreas recortadas', async ({ page }) => {
  await page.setViewportSize({ width: 844, height: 390 })
  await page.goto('/?qa-iphone=1&view=work&safe-top=0&safe-right=62&safe-bottom=21&safe-left=62&theme=light')
  const table = page.getByRole('region', { name: 'Registos de trabalho' })
  await expect(table.getByText('180 registos de 180')).toBeVisible()
  const header = page.locator('.app-shell-header')
  const menuButton = header.getByRole('button', { name: 'Abrir navegação' })
  const menuBox = await menuButton.boundingBox()
  expect(menuBox).not.toBeNull()
  expect(menuBox!.x).toBeGreaterThanOrEqual(62)
  await menuButton.click()
  const firstNavButton = page.getByRole('complementary', { name: 'Navegação principal' }).getByRole('button', { name: 'Visão Geral' })
  const navBox = await firstNavButton.boundingBox()
  expect(navBox).not.toBeNull()
  expect(navBox!.y).toBeGreaterThanOrEqual(0)
  await page.getByRole('button', { name: 'Fechar navegação' }).click({ position: { x: 500, y: 200 } })
  const clientHeader = table.getByRole('columnheader', { name: /Cliente/ })
  await clientHeader.getByRole('button', { name: 'Filtrar…' }).click()
  const panel = page.getByRole('dialog', { name: 'Filtro Cliente' })
  const box = await panel.boundingBox()
  expect(box).not.toBeNull()
  expect(box!.x).toBeGreaterThanOrEqual(62+8)
  expect(box!.x+box!.width).toBeLessThanOrEqual(844-62-8)
  expect(box!.y).toBeGreaterThanOrEqual(8)
  expect(box!.y+box!.height).toBeLessThanOrEqual(390-21-8)
  await page.keyboard.press('Escape')
  await page.getByRole('button', { name: 'Criar novo registo' }).click()
  const modal = page.getByRole('dialog', { name: 'Criar movimento' })
  const formBox = await modal.locator('form').boundingBox()
  expect(formBox).not.toBeNull()
  expect(formBox!.x).toBeGreaterThanOrEqual(62)
  expect(formBox!.x+formBox!.width).toBeLessThanOrEqual(844-62)
  expect(formBox!.y+formBox!.height).toBeLessThanOrEqual(390-21)
})
...
test('criação de movimento no iPhone mantém as acções visíveis durante o scroll', async ({ page }) => {
  ...
  for (const treatment of ['Facturável','Facturado','Pago','Avença']) await expect(dialog.getByLabel(treatment,{exact:true})).toBeVisible()
  await dialog.getByLabel('Pago',{exact:true}).check()
  for (const cumulative of ['Facturável','Facturado','Pago']) await expect(dialog.getByLabel(cumulative,{exact:true})).toBeChecked()
  await expect(dialog.getByLabel('Avença',{exact:true})).not.toBeChecked()
  ...
  const horizontalFit=await dialog.evaluate(element=>({dialogClient:element.clientWidth,dialogScroll:element.scrollWidth,pageClient:document.documentElement.clientWidth,pageScroll:document.documentElement.scrollWidth,fields:[...element.querySelectorAll('input,select,textarea,button')].every(node=>{const box=node.getBoundingClientRect();return box.left>=0&&box.right<=document.documentElement.clientWidth+1})}))
  expect(horizontalFit.dialogScroll).toBeLessThanOrEqual(horizontalFit.dialogClient)
  expect(horizontalFit.pageScroll).toBeLessThanOrEqual(horizontalFit.pageClient)
  expect(horizontalFit.fields).toBe(true)
