...
test('prestação abre por duplo clique e só grava a própria prestação na confirmação',async({page})=>{
 ...
 const retainerCard=page.getByRole('listitem').filter({has:page.getByRole('heading',{name:'Cliente Demonstração Alfa'})})
 await expect(retainerCard).toContainText('Por facturar')
 await expect(retainerCard).toContainText('Prestações geradas ainda sem pagamento')
 await expect(retainerCard).toContainText('100,00')
 await expect(retainerCard).toContainText('Valor por período de facturação')
 await expect(retainerCard).toContainText('Valor anual equivalente')
 await expect(retainerCard).toContainText('1200,00')
 await expect(retainerCard.getByRole('link',{name:'Registos do cliente'})).toHaveAttribute('href',/clientId=00000000-0000-4000-8000-000000000020/)
 await retainerCard.getByRole('button',{name:'Ficha do cliente'}).click()
