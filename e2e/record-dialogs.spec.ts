...
test('prestação abre por duplo clique e só grava a própria prestação na confirmação',async({page})=>{
 ...
 expect(openedUrl.searchParams.get('record')).toBe(retainer.client_id)
 ...
 await expect(compact.getByRole('button',{name:'Cobrança',exact:true}).first()).toBeDisabled()
 await expect(compact.getByRole('button',{name:'Cobrança',exact:true}).first()).toHaveAttribute('title','Não há movimentos facturados e não pagos para este cliente.')
