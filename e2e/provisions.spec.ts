...
test('Provisões calcula o saldo na linha e abre os registos sem emitir notas',async({page})=>{
 ...
 const cards=page.getByRole('region',{name:'Clientes com provisões'})
 await expect(cards).toContainText('Cliente Sintético');await expect(cards).toContainText('Cliente Sem Saldo')
 await expect(cards).toContainText('877,00');await expect(cards).toContainText('Provisão excedida')
 const exhausted=cards.getByRole('listitem').filter({has:page.getByRole('heading',{name:'Cliente Sem Saldo'})})
 await expect(exhausted).toContainText('Posição da provisão após trabalho registado')
 await expect(exhausted).toContainText('-246,00')
 await expect(exhausted.getByRole('link',{name:'Registos do cliente'})).toHaveAttribute('href',/clientId=00000000-0000-4000-8000-000000000021/)
 await cards.getByRole('listitem').filter({has:page.getByRole('heading',{name:'Cliente Sintético'})}).getByRole('button',{name:'Histórico'}).click()
 ...
 await usage.getByText('Ver os registos considerados na provisão (2)',{exact:true}).click()
 await expect(usage).toContainText('Preparação de requerimento')
 await expect(usage).toContainText('Já incluído na nota NH-P-00000001')
...
test('estornar a nota repõe o saldo efectivo na ficha e na linha do cliente',async({page})=>{
 await page.goto('/?qa-iphone=1&qa-demo=1&qa-provisions=1&view=provisions')
 await page.getByRole('listitem').filter({has:page.getByRole('heading',{name:'Cliente Sintético'})}).getByRole('button',{name:'Histórico'}).click()
 ...
 await expect(page.getByRole('region',{name:'Clientes com provisões'})).toContainText('1000,00')
})
...
for(const viewport of [{width:320,height:568},{width:390,height:844},{width:768,height:1024},{width:1440,height:900}]){
 test(`Provisões sem overflow e formulário acessível ${viewport.width}px`,async({page})=>{
  ...
  await page.goto('/?qa-iphone=1&qa-demo=1&qa-provisions=1&view=provisions')
  const cards=page.getByRole('region',{name:'Clientes com provisões'})
  expect(await cards.evaluate(element=>element.scrollWidth<=element.clientWidth+1)).toBe(true)
  await expect(cards.getByRole('listitem').filter({has:page.getByRole('heading',{name:'Cliente Sem Saldo'})})).toContainText('Posição da provisão após trabalho registado')
  await page.getByRole('listitem').filter({has:page.getByRole('heading',{name:'Cliente Sintético'})}).getByRole('button',{name:'Histórico'}).click()
