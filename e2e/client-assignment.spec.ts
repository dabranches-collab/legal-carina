...
test('novo movimento assume a sociedade do cliente, permite alteração e reinicia ao mudar cliente',async({page})=>{
 ...
 await page.route('**/rest/v1/**',async route=>{
  ...
  if(table==='clients'&&url.searchParams.get('select')==='primary_billing_entity_id')result={primary_billing_entity_id:url.searchParams.get('id')===`eq.${id(20)}`?id(2):null}
 ...
 await expect(dialog).toHaveCount(0);expect(saved?.p_billing_entity_id).toBe(id(2))
})
