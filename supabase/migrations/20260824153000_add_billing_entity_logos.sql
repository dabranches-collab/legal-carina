...
-- Estas funções apenas extraem UUIDs do caminho. As políticas de Storage são
-- avaliadas como o utilizador autenticado e precisam de as poder executar.
grant execute on function private.storage_firm_id(text) to authenticated;
...
create policy billing_entity_logos_select on storage.objects for select to authenticated using (
  bucket_id='billing-entity-logos' and exists (
    ...
    where b.firm_id=private.storage_path_uuid(storage.objects.name,1) and b.id=private.storage_path_uuid(storage.objects.name,2)
