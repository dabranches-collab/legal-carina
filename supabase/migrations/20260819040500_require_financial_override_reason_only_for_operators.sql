...
declare e public.work_entries%rowtype;target_client uuid;new_invoiced boolean;new_paid boolean;new_invoice_date date;why text;operator_requires_reason boolean;
begin
 ...
 if operator_requires_reason and (nullif(p_values->>'effective_hourly_rate','')::numeric is distinct from e.effective_hourly_rate or nullif(p_values->>'effective_amount','')::numeric is distinct from e.effective_amount)and btrim(coalesce(p_reason,''))=''then raise exception 'override reason required';end if;
