...
-- The new creation/edit wrappers require the task referrer. Legacy RPCs remain
-- compatible while the published frontend does not yet expose these fields.
...
begin
 ...
 if (p_start is null)<>(p_end is null) or p_start>p_end or p_offset is null or p_offset<0 or p_limit is null or p_limit not between 1 and 500 then raise exception 'Período ou paginação inválidos.';end if;
