...
create index if not exists user_login_credentials_lookup_idx
  on public.user_login_credentials (username)
  where locked_until is null or failed_attempts < 5;

drop trigger if exists user_login_credentials_set_updated_at on public.user_login_credentials;
...
create table if not exists public.billing_entity_financial_permissions (
  id uuid primary key default gen_random_uuid(),
  firm_id uuid not null references public.law_firms(id) on delete cascade,
...
drop trigger if exists billing_entity_financial_permissions_set_updated_at on public.billing_entity_financial_permissions;
