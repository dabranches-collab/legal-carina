...
-- As filas financeiras dos registos incluem o estado financeiro do trabalho
-- a preço fixo ao qual cada registo pertence.
create or replace function public.get_work_attention_counts(
 ...
 p_billing_entity_id uuid default null,p_archive text default null,p_client_type text default null,
...
with memberships as materialized(
 ...
 select w.*,j.is_invoiced fixed_is_invoiced,j.is_paid fixed_is_paid,j.status fixed_status
 from public.work_entries w
 join memberships m on m.firm_id=w.firm_id
 join public.clients c on c.id=w.client_id
 left join public.fixed_fee_jobs j on j.id=w.fixed_fee_job_id
 where (m.privileged or private.has_scope_access(w.firm_id,w.billing_entity_id,w.client_id,w.matter_id,'view'))
 and(p_search is null or btrim(p_search)='' or w.activity_description ilike'%'||p_search||'%' or coalesce(w.observations,'') ilike'%'||p_search||'%' or c.display_name ilike'%'||p_search||'%' or c.client_code ilike'%'||p_search||'%' or coalesce(j.title,'') ilike'%'||p_search||'%')
...
select jsonb_build_object(
 ...
 'uninvoiced',count(*)filter(where (billing_scope='standard' and not is_invoiced and status<>'uncollectible_uninvoiced') or (billing_scope='fixed_fee' and not fixed_is_invoiced and fixed_status<>'cancelled')),
 'unpaid',count(*)filter(where (billing_scope='standard' and is_invoiced and not is_paid and status<>'uncollectible_invoiced') or (billing_scope='fixed_fee' and fixed_is_invoiced and not fixed_is_paid and fixed_status<>'cancelled')),
 'historical',count(*)filter(where billing_scope='standard' and is_paid and invoice_date is null and not exists(select 1 from public.invoice_lines il join public.invoices i on i.id=il.invoice_id where il.work_entry_id=filtered.id and (i.invoice_date is not null or nullif(btrim(i.invoice_number),'') is not null))),
 'retainer',count(*)filter(where billing_scope='retainer')
) from filtered;
$$;

create or replace function public.get_attention_work_entries(
 p_kind text,p_search text default null,p_year integer default null,p_professional_id uuid default null,
 p_billing_entity_id uuid default null,p_archive text default null,p_missing_price boolean default false,
 p_client_type text default null,p_client_id uuid default null,p_missing_society boolean default false
)
...
begin
 ...
 select coalesce(jsonb_agg(item||jsonb_build_object('billing_scope',coalesce(w.billing_scope,'standard'),'fixed_fee_job_id',w.fixed_fee_job_id)),'[]'::jsonb) into filtered
 from jsonb_array_elements(coalesce(payload->'items','[]'::jsonb)) item
 join public.work_entries w on w.id=(item->>'id')::uuid
 left join public.fixed_fee_jobs j on j.id=w.fixed_fee_job_id
 where
  (p_kind='uninvoiced' and ((w.billing_scope='standard' and (item->>'is_invoiced')::boolean=false and item->>'status'<>'uncollectible_uninvoiced') or (w.billing_scope='fixed_fee' and not j.is_invoiced and j.status<>'cancelled'))) or
  (p_kind='unpaid' and ((w.billing_scope='standard' and (item->>'is_invoiced')::boolean=true and (item->>'is_paid')::boolean=false and item->>'status'<>'uncollectible_invoiced') or (w.billing_scope='fixed_fee' and j.is_invoiced and not j.is_paid and j.status<>'cancelled'))) or
