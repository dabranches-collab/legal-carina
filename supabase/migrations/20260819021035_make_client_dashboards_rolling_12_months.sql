...
with mixed_clients as materialized(
  ...
  select generate_series(
    (select value from latest_month)-interval '11 months',
    (select value from latest_month),
    interval '1 month'
  )::date month_start
  ...
  select coalesce(sum(duration_minutes),0) minutes,sum(effective_amount) total,
    ...
    sum(effective_amount) filter(where is_paid) paid,count(*) movements,
    count(distinct client_id) clients,count(distinct professional_id) professionals,
...
select jsonb_build_object(
  ...
  'metrics',jsonb_build_object(
    ...
    'pending',case when t.invoiced is null then null else t.invoiced-coalesce(t.paid,0) end,
    ...
    'unpaidCount',t.unpaid_count,'missingPrice',t.missing_price
  ),
